...
This folder contains all prompts used by code-agent-org-mode, organized by type.
...
Use =M-x code-agent-org-insert-sdd= (or =C-c C-d=) to create an SDD structure:
...
| =lp=       | Literate programming guidelines      |
...
:PROPERTIES:
:CUSTOM_ID: README-default-tags-claude-tags
:END:

Use the =CLAUDE_TAGS= property to automatically include tags for all AI blocks.
Tags are merged with heading tags (heading tags take priority).
...
,* My Literate Code :claude_chat:

,#+begin_src ai
;; This block inherits :lp: from file property
,#+end_src
#+end_example
...
,** Implementation :code:claude_chat:
;; Inherits lp, strict from property + code from heading
#+end_example

** Dynamic Tag Discovery

Tags are discovered automatically from =prompts/tags/*.org=. When you add a new
tag file, it appears in =code-agent-org-insert-block= menu (=C-c C-/ i=).

Each tag file should have:
- =#+TITLE:= - Tag name
- =#+DESCRIPTION:= - Short description (shown in menu)
- =#+PROPERTY: TAG tagname= - The actual tag string
- =* Summary= section - Loaded into system prompt
- =* Details= section - Claude reads via org link when needed
...
M-x code-agent-org-insert-sdd RET "JWT Authentication" RET
...
Prompts are loaded dynamically at runtime from =prompts/tags/TAG.org=.
