...
* Summary
:PROPERTIES:
:CUSTOM_ID: tags-research-summary
:END:

| Property | Value |
|----------+-------|
| Tag | =:research:= |
| Purpose | Explore codebase using parallel agents |
| Output | Write findings to Research Output sibling section |

Research agents: =codebase-locator=, =codebase-analyzer=, =codebase-pattern-finder=, =codebase-online-researcher=.

Full instructions: [[file:prompts/tags/research.org::*Details][Read Details]]

* Details
:PROPERTIES:
:CUSTOM_ID: tags-research-details
:END:
...
*** This Phase Does NOT
- Design solutions or propose architectures (that's Design phase)
- Make recommendations or suggest improvements
- Create TODO items or plan implementation
- Write any code

*** Input
- User's request/question from the Workflow section

*** Output → Used By Design Phase
- Research Output section with factual findings
- Codebase Patterns, Relevant Files, External References
...
Adjust research intensity based on question complexity:

| Depth | When to use | Approach |
|-------+-------------+----------|
| Quick | Simple, localized questions | Direct search, summarize |
