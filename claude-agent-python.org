...
def main() -> None:
    ...
    # Name-based routing: when launched via workspace_launcher, these
    # two env vars pin the bridge to a specific (cmux workspace, org
    # heading) pair so it cannot misroute even if a stale claude
    # process is resumed inside the wrong cmux workspace.  Detection
    # trigger: we are clearly running inside cmux (it set CMUX_WORKSPACE_ID
    # for every pane) but our launcher did NOT run (CMUX_WORKSPACE +
    # WORKSPACE_CUSTOM_ID would be set if it had).  That combination
    # is the 2026-05-21 cross-routing bug — a cmux resume of a saved
    # claude session inherited stale routing env from the original
    # workspace.
    #
    # Response: skip routing and exit 0.  We deliberately do NOT
    # ``sys.exit(2)`` — that would break the Claude CLI hook chain
    # for every pre-existing session that was launched before this
    # check shipped (those sessions never had the launcher set
    # CMUX_WORKSPACE either).  Logging + skip means the session
    # keeps running; the cost is that prompts/responses are dropped
    # rather than misrouted.  Users see the warning, relaunch via
    # workspace_launcher.py once, and the next session has the new
    # env and routes correctly.
    #
    # Tests that exercise the legacy session_id path while themselves
    # running inside cmux (so CMUX_WORKSPACE_ID is inherited from the
    # test runner's shell) opt out by setting WORKSPACE_BRIDGE_TEST_BYPASS=1
    # so they continue to insert into the org buffer.
    if (
        ...
        print(
            ...
            "missing (CMUX_WORKSPACE empty). This usually means cmux "
            "resumed a saved claude session without going through "
            "workspace_launcher.py, so the WORKSPACE_ORG_FILE / "
            "WORKSPACE_SESSION_ID env vars may be stale from the "
            "original launch.  Relaunch claude via the cmux 'New "
            "Claude session' action so the launcher can set "
            "CMUX_WORKSPACE + WORKSPACE_CUSTOM_ID from the current "
            "cmux workspace.  This message is non-fatal; the hook "
            "chain continues.",
            file=sys.stderr,
        )
        sys.exit(0)
