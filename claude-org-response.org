...
Response utilities for claude-org: query-ID generation, header normalization,
...
#+BEGIN_SRC elisp
(require 'cl-lib)
;; Depends on claude-org-session for fontlock-timer buffer-local var
;; Depends on claude-org for defcustom variables (normalize-headers, etc.)
