...
#+begin_src json
{
  "description": "Emacs Claude Agent hooks — permission handling + CLI session persistence",
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
...
def main() -> None:
    ...
    cmux_workspace = (
        os.environ.get("CMUX_WORKSPACE")
        or os.environ.get("ORCA_WORKTREE")
        or ""
    )
...
``EMACS_MCP_URL`` (exported by the base ``export_env_vars``) — so there
is no screen-scraping and no transcript file.  (The org-buffer
response-insertion Elisp function this extension originally called
over MCP on Pi's ``agent_end`` event lived in the now-deleted
``code-agent-org-workspace-bridge.org`` and was removed with the cmux
response-sync trim in commit ``7980a53``; Pi's response-delivery path
needs re-verification against the current Emacs-side API.)
