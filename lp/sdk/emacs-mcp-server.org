...
This variable is set by =code-agent-query= when Docker mode is active.
Since MCP requests arrive asynchronously via HTTP (outside any let-binding
scope), =setq= must be used instead of =let=.  The variable is automatically
cleared when the query completes.

#+BEGIN_SRC elisp
(defvar emacs-mcp-server-path-mappings nil
  "Alist of (HOST-PATH . CONTAINER-PATH) for Docker path translation.
This variable is set by `code-agent-query' when Docker mode is active
and cleared on query completion.  MCP requests use this to translate
container paths to host paths before evaluation, and host paths back to
container paths in results.
...
When `emacs-mcp-server-path-mappings' is bound (by `code-agent-query' in Docker
mode), translates container paths to host paths before evaluation and back again
in results.
