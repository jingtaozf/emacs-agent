...
Uses `process-get' to retrieve state instead of closures (for dynamic binding compat)."
  (let ((state (process-get process 'claude-agent-state)))
    ...
    (when state
      ...
      (claude-agent--process-json-buffer process)
      ;; Handle any remaining content without trailing newline (e.g., "Prompt is too long")
      (let ((remaining (claude-agent--process-state-json-buffer state)))
        (when (and remaining (not (string-empty-p (string-trim remaining))))
          (let ((error-callback (claude-agent--process-state-error-callback state)))
            (claude-agent--debug-log "Remaining non-JSON output: %s" remaining)
            (when error-callback
              (funcall error-callback
                       (list 'claude-agent-cli-error
                             :message (string-trim remaining)
                             :original-error nil))))))
        ...
        (cond
         ...
         ((equal subtype "can_use_tool")
          (let* ((tool-name (plist-get request-data :tool_name))
                 ...
                 (result (claude-agent--run-permission-functions
...
(defun claude-agent-permission-prompt (tool-name tool-input context)
  "Prompt user with y-or-n-p, cache decision per session."
