  ...
  (let ((type (plist-get parsed :type)))
    (cond
     ...
     ((and (string= type "result")
      ...
      (or (plist-get parsed :result)
          (plist-get parsed :error)
          (plist-get parsed :message)
          ;; Check :error_details for session errors like "No conversation found"
          (when-let ((details (plist-get parsed :error_details)))
            (if (stringp details)
                details
              (format "%S" details)))
          ;; Check subtype for specific error types
