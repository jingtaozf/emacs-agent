...
(cl-defstruct (claude-agent--process-state
  ...
  source-label      ; Human-readable label like "5" for Instruction 5
...
SOURCE-LABEL is a human-readable label like \"5\" for Instruction 5 (optional).
...
(cl-defun claude-agent-query (prompt &key options on-message on-token on-error on-complete
                                     session-key source-buffer source-label
                                     source-location-fn source-location-args
...
Returns a string like \"dev.org#5\" or \"#123\" as fallback."
  (let ((buf (claude-agent--process-state-source-buffer state))
        (label (claude-agent--process-state-source-label state))
     ...
     (lambda (_id state)
       (when (and state
         ...
         (let* ((start-time (claude-agent--process-state-start-time state))
                ...
                (identity (claude-agent--format-query-identity state))
                (docker-p (claude-agent--process-state-docker-mode state))
                (docker-tag (if docker-p " [Docker]" "")))
           (push (format "● %s%s  [%s %s]" identity docker-tag spinner elapsed) lines))))
     claude-agent--active-queries)
    (if (= (length lines) 2)
               ...
               (let* ((start-time (claude-agent--process-state-start-time state))
                 ...
                 (when docker-p
                   (insert (propertize " [Docker]"
                                       'face '(:weight bold))))
                 (when label
                   (insert (propertize (format " Instruction %s" label)
