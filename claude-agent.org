...
Hook matchers configure how callbacks are invoked during Claude's tool execution
lifecycle. Each matcher can target specific tools by name pattern (or match all
tools with nil) and includes a configurable timeout for async operations.

The supported hook events correspond to different stages of tool execution:
...
(defun claude-agent--maybe-close-stdin (process parsed)
  "Close stdin if PARSED is a result message.
