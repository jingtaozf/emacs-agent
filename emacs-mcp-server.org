...
(defun emacs-mcp-server--effective-path-mappings ()
  "Return the current effective path mappings.
Returns `emacs-mcp-server-path-mappings' or nil if empty."
  emacs-mcp-server-path-mappings)
#+END_SRC
