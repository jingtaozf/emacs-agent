...
(defconst emacs-mcp-server--builtin-tools
  `(((name . "evalElisp")
     (description . "Evaluate an Emacs Lisp expression and return the result. This is the single tool for all Emacs operations - use Skills to learn how to construct elisp code for specific tasks like buffer manipulation, variable inspection, navigation, etc. IMPORTANT: Each call runs in an isolated context. Point, buffer, narrowing, and window state are restored to the user's editing position after every call. Any navigation you perform (goto-char, re-search-forward, switch-to-buffer) does NOT persist to the next call. You must perform all navigation and action within a single evalElisp call using save-excursion, with-current-buffer, etc.")
