...
This variable is set by =claude-agent-query= when Docker mode is active.
...
When `emacs-mcp-server-path-mappings' is bound (by `claude-agent-query' in Docker
...
loops cannot be interrupted — an inherent Emacs limitation."
  (let* ((raw-code (alist-get 'code params))
    ...
    (unwind-protect
        (setq result
              (claude-agent-with-span "mcp-evalElisp"
                  (list :input (or code "")
                        :oi-kind "TOOL"
                        :code (or code "")) nil
                (condition-case err
...
Runs `claude-agent-invocation-hook' with the invocation details."
  (let ((type (alist-get 'type params))
    ...
    ;; Run the hook if defined (claude-agent.org defines it)
    (when (boundp 'claude-agent-invocation-hook)
      (run-hook-with-args 'claude-agent-invocation-hook type name reason))
