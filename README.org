...
Templates are loaded fresh from =claude-org-templates.el= every time you invoke
the template menu. Edit the file and changes take effect immediately — no restart needed.

*Option 1: Edit the default file*

Edit =claude-org-templates.el= in the package directory directly.
Changes are picked up on the next template insertion.

*Option 2: Use a custom templates file*

Point =claude-org-templates-file= to your own file:

#+BEGIN_SRC elisp :load no
(setq claude-org-templates-file "~/my-templates.el")
#+END_SRC

Your file should contain a single alist as its first readable form:

#+BEGIN_SRC elisp :load no
;; ~/my-templates.el — a data file (not executed, just read)
(("Code Review" . "run code simplifier and code review to the code changes.")
 ("My Custom" . "Please do something custom...")
 ("Dynamic" . my-template-function))  ;; function symbol returning a string
#+END_SRC

*** Writing Template Functions

For dynamic templates (e.g., capturing buffer content), define a function:

#+BEGIN_SRC elisp :load no
(defun my-template--selection ()
  "Template that includes the current selection."
  (let ((sel (buffer-substring-no-properties (region-beginning) (region-end))))
    (format "Please refactor this code:\n\n```\n%s\n```\n" sel)))

;; Then add to templates:
;; ("Refactor Selection" . my-template--selection)
#+END_SRC
