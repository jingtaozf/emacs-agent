...
** Knowledge Extraction (Claudeception)
:PROPERTIES:
:CUSTOM_ID: ref-knowledge-extraction
:END:

Post-execution analysis that extracts reusable skills and rules from completed work.
Inspired by [[https://github.com/blader/Claudeception][Claudeception]], this feature helps build a knowledge base over time.

*** Enabling Extraction

Add the hook to your Emacs config:

#+BEGIN_SRC elisp :load no
;; Enable knowledge extraction (skills and rules) after AI block execution
...
After each execution completes, Claude analyzes the work and extracts:

| Type | Purpose | Save Location |
|------+---------+---------------|
| Skill | How-to solve a problem | =~/.claude/skills/emacs-agent/[name]/SKILL.md= |
| Rule | Constraint or pattern to follow | =~/.claude/rules/emacs-agent/[name].md= |

Extraction only happens when Claude is *very certain* the knowledge is:
...
After execution, a "Skill/Rule Extraction" section is appended:
...
- *Trigger*: "Too many connections" error in serverless
- *Solution*: Use connection pooling with pgBouncer...
- *Save to*: =~/.claude/skills/emacs-agent/prisma-connection-pool-fix/SKILL.md=

,** Rules
- *Name*: =always-use-markers.md=
- *Rule*: Use markers instead of point for async operations
- *Rationale*: Point can change during async execution
- *Save to*: =~/.claude/rules/emacs-agent/always-use-markers.md=

,** Summary
- Skills: prisma-connection-pool-fix
- Rules: always-use-markers
#+END_EXAMPLE

If no extractable knowledge is found, it responds: "No new skills or rules extracted."

*** Loop Execution Timing

For loops (=:loop N=), configure when extraction runs:

#+BEGIN_SRC elisp :load no
;; Extract once after all iterations (default, recommended)
(setq claude-org-loop-extraction-timing 'after-all)

;; Extract after each iteration
(setq claude-org-loop-extraction-timing 'after-each)
#+END_SRC

| Timing | When | Best For |
|--------+------+----------|
| =after-all= | After final iteration | Most cases, avoids redundancy |
| =after-each= | After every iteration | Exploratory work, diverse iterations |

*** Customizing the Prompt

Edit =prompts/skills/claudeception.org= to customize extraction behavior.
