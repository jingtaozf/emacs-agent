...
The presentation layer (=claude-org.org=) never calls backend internals directly.
...
|---------+--------------+--------|
| *cmux* (default) | Runs Claude Code in a [[https://github.com/manaflow-ai/cmux][cmux]] terminal multiplexer. Full TUI, response capture via hooks. | Recommended |
| *json-stream* | Pipes prompt via =--print --output-format stream-json=. | Deprecated |
...
execute an AI block, =claude-org-cmux--call= will error with a clear message.
...
|----------+---------+---------|
| =claude-org-cmux-cli-path= | ="cmux"= | Path to the cmux CLI executable |
| =claude-org-cmux-socket-path= | =nil= | Unix socket path (nil = auto-detect) |
| =claude-org-cmux-launch-command= | ='claude-workspace= | How to launch Claude Code in cmux |

Example for a custom cmux install:

#+begin_src elisp
(setq claude-org-cmux-cli-path "/opt/cmux/bin/cmux")
#+end_src
...
another the default json-stream Claude, all in the same file.
...
Use =C-c C-/= then =d= (or =M-x claude-org-insert-sdd=):
...
(setq claude-org-sdd-docs-format "md")  ;; "org" or "md"
#+END_SRC
...
(setq claude-org-prompts-directory "/path/to/my/prompts")
#+END_SRC
...
survive package updates. Set =claude-org-extra-snippet-dirs= in your Emacs config:

#+BEGIN_SRC elisp :load no
(setq claude-org-extra-snippet-dirs '("~/my-snippets"))
#+END_SRC
...
(setq claude-org-scheduled-files
      '("~/org/tasks.org" "~/org/automation.org"))
(claude-org-scheduled-start)
#+END_SRC

*Note:* If =claude-org-scheduled-files= is nil, the scheduler automatically uses
...
|----------+-------------|
| =claude-org-scheduled-start= | Start the scheduler timer |
| =claude-org-scheduled-stop= | Stop the scheduler timer |
| =claude-org-scheduled-scan-all= | Manually scan and update scheduled blocks |
