...
Version note: this layer calls five *private* =pi-coding-agent=
functions, because the public API cannot yet address a named session
(see [[file:tasks/pi-topic-embed-map.md][tasks/pi-topic-embed-map.md]]).  They are checked by name at
startup, so an incompatible upgrade fails loudly instead of silently
misbehaving.  Verified against 2.6.0 and 2.7.0.
...
- Five private =pi-coding-agent= functions are in the dependency path;
  four upstream PRs would retire them ([[file:tasks/pi-topic-embed-map.md][the map]] names each ask).
