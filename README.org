...
| *iTerm2* | Runs Claude Code in an iTerm2 pane via AppleScript. macOS only. | Stable |
...
| | =T= | Native terminal | Open interactive Claude Code terminal |
...
:PROPERTIES:
:CUSTOM_ID: ref-native-terminal-backend
:END:

The native backend opens a persistent interactive Claude Code terminal (=vterm= or
=eat=) instead of capturing responses into the org buffer. You interact directly
with Claude's TUI while the org file provides the system prompt context.

**** Enabling Native Mode

Set the =CLAUDE_BACKEND= property to =native=:
...
,* Project Guidelines :system_prompt:
Context about your project goes here.

,** Work
,#+begin_src ai
Research the authentication module.
,#+end_src
#+END_EXAMPLE

Press =C-c C-c= on the =ai= block — a terminal opens with the system prompt
loaded, and the query is pasted automatically. Or open the terminal from the
menu: =C-c C-/ T=.

**** Terminal Emulator

The default terminal is =vterm=. To use =eat= instead, set the
=CLAUDE_NATIVE_TERMINAL= property:

#+BEGIN_EXAMPLE
,#+PROPERTY: CLAUDE_NATIVE_TERMINAL eat
#+END_EXAMPLE

Or set globally in your Emacs config:

#+BEGIN_SRC elisp
(setq claude-org-native-terminal-backend 'eat)
#+END_SRC

| Property | Values | Default |
|----------+--------+---------|
| =CLAUDE_BACKEND= | =json-stream=, =claude-cli=, =native= | =json-stream= |
| =CLAUDE_NATIVE_TERMINAL= | =vterm=, =eat= | =vterm= |

**** Behavior

- The terminal buffer stays alive until you close it manually
- Subsequent =C-c C-c= on =ai= blocks reuses the existing terminal
- System prompts (=:system_prompt:= sections, behavior prompts) are passed via =--system-prompt=
- Responses appear in the terminal, not in the org buffer
- Hook system handles permissions (PreToolUse) and buffer revert (PostToolUse)
- If you kill the terminal and reopen it (=C-c C-c= or =C-c C-/ T=), the
  previous conversation resumes automatically via =--resume <session-id>=
- Keybindings in the terminal: =C-c C-q= (kill), =C-c C-r= (refresh)
