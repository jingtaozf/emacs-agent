...
+-------------------------+-------------------------+
              ...
              claude-agent-backend-filter-callbacks
                          |
+-------------------------+-------------------------+
|         Backend Protocol (claude-agent.org)        |
|  cl-defgeneric dispatch: 8 generic functions      |
+-------+-------------------+-----------------------+
        |                   |
+-------+-------+   +-------+-------+   +-------+-------+
| JSON Stream   |   | Claude CLI    |   | Native        |
| Backend       |   | Backend (eat) |   | Terminal      |
| CLI subprocess|   | Automated TUI |   | (vterm/eat)   |
+---------------+   +---------------+   +---------------+
#+end_example

The presentation layer (=code-agent-org.org=) never calls backend internals directly.
All communication goes through the =claude-agent-backend-*= generic protocol.
New backends can be added by implementing 8 =cl-defmethod= specializations.
...
| *tmux* | Runs Claude Code in a detached tmux session. Same TUI model as cmux, cross-platform (Linux + macOS). Phase 4 scaffolding — core loop works, cmux-specific niceties (workspace colour, sidebar) not available. | Early / cross-platform |
...
SDD stores all knowledge artifacts in the =docs/= folder, not in the notebook:
...
Each subdirectory has an =INDEX.md= tracking all docs with status (draft/complete/approved).
...
2. The command creates:
   ...
   - =INDEX.md= entries in both directories
