;;; test-code-agent-org-cancel-race.el --- Integration test for cancel/complete race condition -*- lexical-binding: t -*-
   ...
   (lambda (org-file)
     (with-current-buffer (find-file-noselect org-file)
       ...
       (let ((session-key (code-agent-org-current-session-key))
         ...
         (goto-char block-pos)
         (should (equal (code-agent-org--get-exec-status) "cancelled"))
         ...
         (goto-char block-pos)
         (let ((final-status (code-agent-org--get-exec-status)))
   ...
   (lambda (org-file)
     (with-current-buffer (find-file-noselect org-file)
       ...
       (let ((session-key (code-agent-org-current-session-key))
         ...
         (goto-char block-pos)
         (let ((status-after-cancel (code-agent-org--get-exec-status)))
           (should (equal status-after-cancel "cancelled"))))))))

(provide 'test-code-agent-org-cancel-race)
;;; test-code-agent-org-cancel-race.el ends here
