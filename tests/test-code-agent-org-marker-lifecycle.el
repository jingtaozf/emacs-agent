;;; test-code-agent-org-marker-lifecycle.el --- TDD Tests for Marker Lifecycle -*- lexical-binding: t; -*-
...
;; Note: code-agent-org.org is loaded via Makefile
...
(ert-deftest test-marker-valid-p-nil-marker ()
  ...
  (should-not (code-agent-org--marker-valid-p nil)))

(ert-deftest test-marker-valid-p-non-marker ()
  "TDD: Non-marker object should be invalid."
  :tags '(:unit :fast :stable :isolated :marker :tdd)
  (should-not (code-agent-org--marker-valid-p 42))
  (should-not (code-agent-org--marker-valid-p "string"))
  (should-not (code-agent-org--marker-valid-p '(1 2 3))))
...
(ert-deftest test-marker-valid-p-no-buffer ()
  ...
  (let ((marker (make-marker)))
    ...
    (should-not (code-agent-org--marker-valid-p marker))))
...
(ert-deftest test-marker-valid-p-valid-marker ()
  ...
  (let* ((buf (generate-new-buffer "*test-marker*"))
         (marker (with-current-buffer buf
                   ...
                   (point-marker))))
    (unwind-protect
        (should (code-agent-org--marker-valid-p marker))
      (kill-buffer buf))))
    ...
    (let ((session-key "test-session"))
      ...
      (let* ((buf (generate-new-buffer "*dying-buffer*"))
        ...
        (let ((new-marker (point-marker))
          ...
          (unwind-protect
              (progn
                (setq result (code-agent-org--queue-block
                              session-key
  ...
  (with-temp-buffer
    ...
    (let ((session-key "test-session"))
      ...
      (let* ((buf (generate-new-buffer "*dying-buffer*"))
        ...
        (let ((block (code-agent-org--dequeue-block session-key)))
...
(ert-deftest test-handle-token-handles-killed-buffer ()
  ...
  (let ((session-key "test-session")
    ...
    (let* ((buf (generate-new-buffer "*dying-buffer*"))
      ...
      (condition-case err
          (code-agent-org--handle-token session-key "test token")
...
(ert-deftest test-set-exec-status-invalid-marker ()
  ...
  (let ((buf (generate-new-buffer "*dying-buffer*"))
    ...
    (let ((marker (with-current-buffer buf
      ...
      (condition-case err
          (code-agent-org--set-exec-status "executing" marker)
        (error (setq error-occurred t)))
      ;; Should not have crashed
      (should-not error-occurred))))

(provide 'test-code-agent-org-marker-lifecycle)
;;; test-code-agent-org-marker-lifecycle.el ends here
