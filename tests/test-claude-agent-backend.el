...
(ert-deftest test-backend-json-struct-type-field ()
  "JSON backend type field should be :json-stream."
  :tags '(:unit :fast :stable :isolated :backend :phase-1)
  (let ((backend (claude-agent-json-backend--create)))
    (should (eq :json-stream (claude-agent-backend-type backend)))))
...
(ert-deftest test-backend-json-struct-slots ()
  ...
  (let ((backend (claude-agent-json-backend--create
                  ...
                  :persistent-p nil)))
    (should (equal "test-key" (claude-agent-json-backend-session-key backend)))
    (should-not (claude-agent-json-backend-persistent-p backend))
    (should-not (claude-agent-json-backend-process-state backend))
    (should-not (claude-agent-json-backend-client backend))))

(ert-deftest test-backend-json-struct-persistent-mode ()
  "JSON backend should support persistent-p flag."
  :tags '(:unit :fast :stable :isolated :backend :phase-1)
  (let ((backend (claude-agent-json-backend--create :persistent-p t)))
    (should (claude-agent-json-backend-persistent-p backend))))
...
(ert-deftest test-json-backend-classify-error-context-limit ()
...
(ert-deftest test-json-backend-classify-error-session-expired ()
...
(ert-deftest test-json-backend-classify-error-non-string ()
...
(ert-deftest test-json-backend-supports-p-capabilities ()
...
(ert-deftest test-json-backend-supports-p-persistent-client ()
  "JSON backend persistent-client capability depends on persistent-p flag."
  :tags '(:unit :fast :stable :isolated :backend :phase-2)
  (let ((non-persistent (claude-agent-json-backend--create :persistent-p nil))
        (persistent (claude-agent-json-backend--create :persistent-p t)))
...
(ert-deftest test-json-backend-active-p-with-process-state ()
  ...
  (let ((backend (claude-agent-json-backend--create)))
    ...
    (let ((closed-state (claude-agent--make-process-state :request-id "closed-req"
                                                          :closed t)))
      (setf (claude-agent-json-backend-process-state backend) closed-state)
      (should-not (claude-agent-backend-active-p backend)))))

(ert-deftest test-json-backend-session-id-from-session-key ()
  "JSON backend session-id should use session-key to look up SDK UUID."
  :tags '(:unit :fast :stable :isolated :backend :phase-2)
  (let ((backend (claude-agent-json-backend--create :session-key "test-session")))
...
(ert-deftest test-json-backend-cancel-nil-handle ()
...
(ert-deftest test-json-backend-cleanup-no-state ()
...
(ert-deftest test-json-backend-send-input-no-state ()
