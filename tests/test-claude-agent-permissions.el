...
(ert-deftest test-org-permission-protect-blocks-edit ()
  ...
  (let ((result (code-agent-org-permission-protect-org
...
(ert-deftest test-org-permission-protect-handles-path-variations ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions :org)
  ;; Should block
  (should (code-agent-org-permission-protect-org
           "Edit" '(:file_path "file.org") nil))
  (should (code-agent-org-permission-protect-org
           "Write" '(:file_path "/path/to/file.ORG") nil))
  ;; Should allow (not .org)
  (should-not (code-agent-org-permission-protect-org
...
(ert-deftest test-org-install-protection ()
  ...
  (let ((claude-agent-permission-functions nil))
    ...
    (should (memq #'code-agent-org-permission-protect-org
                  claude-agent-permission-functions))
    ;; Clean up
...
(ert-deftest test-org-remove-protection ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions :org)
  (let ((claude-agent-permission-functions
         (list #'code-agent-org-permission-protect-org
               #'claude-agent-permission-prompt)))
    ;; Simulate no code-agent-org-mode buffers
    (let ((code-agent-org-mode nil))
      (code-agent-org--remove-protection)
      (should-not (memq #'code-agent-org-permission-protect-org
                        claude-agent-permission-functions)))))
...
(ert-deftest test-bash-modifies-org-sed-inplace ()
  ...
  (should (code-agent-org--bash-modifies-org-p "sed -i 's/old/new/g' file.org"))
  (should (code-agent-org--bash-modifies-org-p "sed -i.bak 's/x/y/' notes.org"))
  (should (code-agent-org--bash-modifies-org-p "sed --in-place 's/a/b/' test.org")))

(ert-deftest test-bash-modifies-org-perl-inplace ()
  "Test that perl -i on .org files is detected."
  :tags '(:unit :fast :stable :isolated :permissions :org :bash)
  (should (code-agent-org--bash-modifies-org-p "perl -i -pe 's/old/new/' file.org"))
  (should (code-agent-org--bash-modifies-org-p "perl -pi -e 's/x/y/' notes.org")))

(ert-deftest test-bash-modifies-org-redirect ()
  "Test that output redirection to .org files is detected."
  :tags '(:unit :fast :stable :isolated :permissions :org :bash)
  (should (code-agent-org--bash-modifies-org-p "echo 'content' > file.org"))
  (should (code-agent-org--bash-modifies-org-p "cat tmp >> notes.org"))
  (should (code-agent-org--bash-modifies-org-p "echo hi > /path/to/file.org"))
  (should (code-agent-org--bash-modifies-org-p "cmd 2> error.org")))

(ert-deftest test-bash-modifies-org-cp-mv ()
  "Test that cp/mv to .org files is detected."
  :tags '(:unit :fast :stable :isolated :permissions :org :bash)
  (should (code-agent-org--bash-modifies-org-p "cp temp.txt file.org"))
  (should (code-agent-org--bash-modifies-org-p "mv draft.txt final.org"))
  (should (code-agent-org--bash-modifies-org-p "cp -f source target.org"))
  (should (code-agent-org--bash-modifies-org-p "install -m 644 src dest.org")))

(ert-deftest test-bash-modifies-org-tee ()
  "Test that tee to .org files is detected."
  :tags '(:unit :fast :stable :isolated :permissions :org :bash)
  (should (code-agent-org--bash-modifies-org-p "echo x | tee file.org"))
  (should (code-agent-org--bash-modifies-org-p "cmd | tee -a notes.org")))

(ert-deftest test-bash-modifies-org-temp-swap ()
  "Test that temp file swap pattern is detected."
  :tags '(:unit :fast :stable :isolated :permissions :org :bash)
  (should (code-agent-org--bash-modifies-org-p "awk '{print}' f > tmp && mv tmp file.org"))
  (should (code-agent-org--bash-modifies-org-p "sort f > t && mv t notes.org")))
...
(ert-deftest test-bash-modifies-org-allows-readonly ()
  ...
  (should-not (code-agent-org--bash-modifies-org-p "cat file.org"))
  (should-not (code-agent-org--bash-modifies-org-p "grep pattern file.org"))
  (should-not (code-agent-org--bash-modifies-org-p "head -n 10 notes.org"))
  (should-not (code-agent-org--bash-modifies-org-p "tail -f log.org"))
  (should-not (code-agent-org--bash-modifies-org-p "wc -l *.org"))
  (should-not (code-agent-org--bash-modifies-org-p "ls *.org"))
  (should-not (code-agent-org--bash-modifies-org-p "find . -name '*.org'"))
  ;; sed without -i is read-only (outputs to stdout)
  (should-not (code-agent-org--bash-modifies-org-p "sed 's/x/y/' file.org")))

(ert-deftest test-bash-modifies-org-no-org-reference ()
  "Test that commands without .org reference are allowed."
  :tags '(:unit :fast :stable :isolated :permissions :org :bash)
  (should-not (code-agent-org--bash-modifies-org-p "echo 'hello' > file.txt"))
  (should-not (code-agent-org--bash-modifies-org-p "sed -i 's/x/y/' file.txt"))
  (should-not (code-agent-org--bash-modifies-org-p "cp a.txt b.txt"))
  (should-not (code-agent-org--bash-modifies-org-p nil))
  (should-not (code-agent-org--bash-modifies-org-p "")))
...
(ert-deftest test-bash-modifies-org-edge-cases ()
  ...
  (should (code-agent-org--bash-modifies-org-p "echo x > \"my file.org\""))
  (should (code-agent-org--bash-modifies-org-p "cp tmp 'notes.org'"))
  ;; .org.bak should NOT be blocked (not a .org file)
  (should-not (code-agent-org--bash-modifies-org-p "cp file.org file.org.bak")))
