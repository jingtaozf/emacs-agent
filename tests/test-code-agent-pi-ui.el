;;; test-code-agent-pi-ui.el --- Tests for Pi UI Phase 1 -*- lexical-binding: t; -*-

;;; Commentary:

;; Smoke + live tests for `code-agent-pi-ui.org' — the Phase 1
...
(let* ((root (or (and (boundp 'test-code-agent-project-root)
  ...
  (unless (featurep 'code-agent-pi-ui)
    (literate-elisp-load (expand-file-name "lp/backend/code-agent-pi-ui.org" root))))

(require 'code-agent-pi-backend)
(require 'code-agent-pi-ui)
(require 'pi-test-helpers)


;; --- Smoke ---

(ert-deftest code-agent-pi-ui-smoke--symbols ()
  "Public symbols are defined after load."
  :tags '(:smoke :pi-ui)
  (dolist (s '(code-agent-pi-ui-menu
...
(ert-deftest code-agent-pi-ui-smoke--menu-prefix ()
  "Transient submenu is registered as a prefix."
  :tags '(:smoke :pi-ui)
  (should (get 'code-agent-pi-ui-menu 'transient--prefix)))

(ert-deftest code-agent-pi-ui-smoke--extension-ui-handler-installed ()
  "Loading pi-ui replaces the auto-cancel default with the rich handler."
  :tags '(:smoke :pi-ui)
  (should (eq code-agent-pi-extension-ui-handler
              #'code-agent-pi-ui--extension-ui-handler)))

(ert-deftest code-agent-pi-ui-smoke--current-backend-errors-cleanly ()
  "Outside a session, `--current-backend' raises a clear user-error."
  :tags '(:smoke :pi-ui)
  (let ((err (should-error (code-agent-pi-ui--current-backend)
                           :type 'user-error)))
...
(ert-deftest code-agent-pi-ui-live--cycle-thinking-mutates-state ()
...
RPC pipe round-trips and Pi's reported state shifts."
  ...
  (test-pi-ui--with-ready-backend b
    (let* ((level-before
            (map-elt (code-agent-pi-ui--unwrap
                      (code-agent-pi-ui--call b '((type . "get_state"))))
                     'thinkingLevel)))
      ;; cycle forward — Pi guarantees a transition (or wraps around).
      (code-agent-pi-ui--unwrap
       (code-agent-pi-ui--call b '((type . "cycle_thinking_level"))))
      ...
      (code-agent-pi-ui--unwrap
       (code-agent-pi-ui--call b '((type . "cycle_thinking_level"))))
      (let ((level-after (map-elt (code-agent-pi-ui--unwrap
                                   (code-agent-pi-ui--call b '((type . "get_state"))))
                                  'thinkingLevel)))
...
(ert-deftest code-agent-pi-ui-live--steer-and-followup-accepted ()
...
(ert-deftest code-agent-pi-ui-live--new-session-mints-fresh-id ()
  "`new_session' RPC produces a sessionId different from the one
get_state reports before the call."
  :tags '(:pi-ui-live)
  (skip-unless (test-pi--available-p))
  (test-pi-ui--with-ready-backend b
    (let* ((sid-before (map-elt (code-agent-pi-ui--unwrap
                                 (code-agent-pi-ui--call b '((type . "get_state"))))
                                'sessionId)))
      (code-agent-pi-ui--unwrap
       (code-agent-pi-ui--call b '((type . "new_session"))))
      (let ((sid-after (map-elt (code-agent-pi-ui--unwrap
                                 (code-agent-pi-ui--call b '((type . "get_state"))))
                                'sessionId)))
...
(ert-deftest code-agent-pi-ui-live--set-session-name-roundtrip ()
  ...
  (test-pi-ui--with-ready-backend b
    (let ((target (format "pi-ui-test-%d" (random 99999))))
      (code-agent-pi-ui--unwrap
       (code-agent-pi-ui--call b `((type . "set_session_name") (name . ,target))))
      (let* ((state (code-agent-pi-ui--unwrap
                     (code-agent-pi-ui--call b '((type . "get_state")))))
             (name (map-elt state 'sessionName)))
        (should (equal target name))))))


(ert-deftest code-agent-pi-ui-live--get-available-models-returns-list ()
  "`get_available_models' returns a non-empty vector — the data
backing `code-agent-pi-ui-pick-model''s completing-read."
  :tags '(:pi-ui-live)
  (skip-unless (test-pi--available-p))
  (test-pi-ui--with-ready-backend b
    (let* ((data (code-agent-pi-ui--unwrap
                  (code-agent-pi-ui--call b '((type . "get_available_models")))))
...
(ert-deftest code-agent-pi-ui-live--set-model-roundtrip ()
  "`set_model' with valid provider+id mutates get_state.model.id."
  :tags '(:pi-ui-live)
  (skip-unless (test-pi--available-p))
  (test-pi-ui--with-ready-backend b
    ;; Fetch model list, pick the first one that differs from current.
    (let* ((avail (map-elt (code-agent-pi-ui--unwrap
                            (code-agent-pi-ui--call b '((type . "get_available_models"))))
                           'models))
           (current (map-elt (code-agent-pi-ui--unwrap
                              (code-agent-pi-ui--call b '((type . "get_state"))))
                             'model))
      ...
      (code-agent-pi-ui--unwrap
       (code-agent-pi-ui--call
        b `((type . "set_model")
            (provider . ,(map-elt different 'provider))
            (modelId . ,(map-elt different 'id)))))
      (let* ((after (code-agent-pi-ui--unwrap
                     (code-agent-pi-ui--call b '((type . "get_state")))))
             (after-id (map-elt (map-elt after 'model) 'id)))
        (should (equal after-id (map-elt different 'id)))))))


(ert-deftest code-agent-pi-ui-live--get-session-stats-returns-payload ()
  "`get_session_stats' returns an alist with at least one numeric
field — the data `code-agent-pi-ui-stats' echoes."
  :tags '(:pi-ui-live)
  (skip-unless (test-pi--available-p))
  (test-pi-ui--with-ready-backend b
    (let ((data (code-agent-pi-ui--unwrap
                 (code-agent-pi-ui--call b '((type . "get_session_stats"))))))
      (should (listp data))
...
(ert-deftest code-agent-pi-ui-live--export-html-creates-file ()
...
(ert-deftest code-agent-pi-ui-live--abort-during-stream ()
...
the dispatcher unwinds the in-flight callbacks."
  ...
  (test-pi--with-backend b
    (let ((on-complete nil)
      ...
      (code-agent-backend-cancel b "ui-abort-handle")
...
(ert-deftest code-agent-pi-ui-live--compact-after-exchange ()
...
(ert-deftest code-agent-pi-ui-live--extension-ui-handler-roundtrip ()
...
(ert-deftest code-agent-pi-ui-live--list-session-files-returns-list ()
  "Drive one micro-exchange + force a session-id capture; verify the
session file shows up under `code-agent-pi-ui-sessions-dir/<cwd-hash>/'.
  ...
  (test-pi--with-backend b
    ...
    (let* ((default-directory (file-name-as-directory
                               (expand-file-name "tests/e2e/org/"
                                                  default-directory)))
           (sub (code-agent-pi-ui--cwd-hash default-directory))
           (dir (expand-file-name sub code-agent-pi-ui-sessions-dir))
      ...
      (setf (code-agent-pi-backend-cwd b) default-directory)
      (let ((done nil))
        (code-agent-backend-query
         b "Reply OK."
         (list :on-token (lambda (_d) nil)
        ...
        (should (test-pi--wait-until (lambda () done) 60))
        (should (eq done t)))
      (let* ((files (code-agent-pi-ui--list-session-files default-directory))
             (after-count (length files)))
        (should files)
        (should (> after-count (or before 0)))
        ;; The newest file should parse cleanly to a header alist.
        (let ((hdr (code-agent-pi-ui--read-session-header (car files))))
          (should hdr)
          (should (stringp (map-elt hdr 'id))))))))


(ert-deftest code-agent-pi-ui-live--cwd-hash-shape ()
  "`--cwd-hash' produces the same encoding Pi uses for session dirs:
slashes → dashes, leading + trailing dashes wrap the path."
  :tags '(:smoke :pi-ui)
  (should (equal (code-agent-pi-ui--cwd-hash "/home/user/proj")
                 "--home-user-proj-"))
  (should (equal (code-agent-pi-ui--cwd-hash "/tmp/")
                 "--tmp--"))
  (let ((default-directory "/home/user/proj/"))
    (should (string-prefix-p "--home-user-proj-"
                              (code-agent-pi-ui--cwd-hash default-directory)))))


(ert-deftest code-agent-pi-ui-live--extension-ui-set-status-surfaces ()
...
(ert-deftest code-agent-pi-ui-live--extension-ui-set-widget-acks ()
...
resolves and the LLM isn't stuck."
  ...
  (skip-unless (test-pi--available-p))
  (test-pi-ui--with-ready-backend b
    (setf (code-agent-pi-backend-session-key b) "widget-test")
    (let ((sent nil))
      (cl-letf (((symbol-function 'code-agent-pi--send)
...
(ert-deftest code-agent-pi-ui-live--login-command-shells-out ()
  "`code-agent-pi-ui-login' should call `async-shell-command' with
...
(provide 'test-code-agent-pi-ui)
;;; test-code-agent-pi-ui.el ends here
