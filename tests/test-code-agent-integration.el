;;; test-code-agent-integration.el --- Integration tests for code-agent.org -*- lexical-binding: t; -*-
...
;; Integration tests for code-agent.org (core SDK module)
...
When `code-agent-auto-recovery' is nil, killing the process
...
answers via `code-agent-ask-user-question-mock-answers', and verifies
Claude's response references the selected answer."
  ...
  (let* ((code-agent-ask-user-question-mock-answers
    ...
    (let ((original-fn (symbol-function 'code-agent-permission-ask-user-question)))
      (cl-letf (((symbol-function 'code-agent-permission-ask-user-question)
        ...
        (code-agent-query
         ...
         :on-message (lambda (msg)
                       (when (code-agent-assistant-message-p msg)
                         (setq response (concat response (or (code-agent-extract-text msg) "")))))
         :on-complete (lambda (_result)
                        (setq completed t)))
...
(provide 'test-code-agent-integration)
;;; test-code-agent-integration.el ends here
