;;; test-code-agent-chat-backend.el --- Tests for chat backend integration -*- lexical-binding: t; -*-

;;; Commentary:
;; TDD tests for wiring code-agent-chat to the backend system.
...
;; ---------------------------------------------------------------------------
;; F1: defcustom code-agent-chat-backend-type
;; ---------------------------------------------------------------------------

(ert-deftest test-chat-f1-backend-type-defcustom-exists ()
  "code-agent-chat-backend-type defcustom should exist."
  :tags '(:unit :fast :stable :isolated :chat-backend :f1)
  (should (boundp 'code-agent-chat-backend-type)))

(ert-deftest test-chat-f1-backend-type-default-is-claude-code ()
  "code-agent-chat-backend-type should default to claude-code."
  :tags '(:unit :fast :stable :isolated :chat-backend :f1)
  (should (eq 'code-agent code-agent-chat-backend-type)))

;; ---------------------------------------------------------------------------
;; F2: Buffer-local code-agent-chat--backend
;; ---------------------------------------------------------------------------

(ert-deftest test-chat-f2-backend-var-is-buffer-local ()
  "code-agent-chat--backend should be buffer-local after mode activation."
  :tags '(:unit :fast :stable :isolated :chat-backend :f2)
  (with-temp-buffer
    (let ((code-agent-chat--backend nil))
      ;; defvar-local makes it automatically buffer-local
      (should (local-variable-if-set-p 'code-agent-chat--backend)))))

(ert-deftest test-chat-f2-query-handle-var-is-buffer-local ()
  "code-agent-chat--query-handle should be buffer-local after mode activation."
  :tags '(:unit :fast :stable :isolated :chat-backend :f2)
  (with-temp-buffer
    (let ((code-agent-chat--query-handle nil))
      (should (local-variable-if-set-p 'code-agent-chat--query-handle)))))

;; ---------------------------------------------------------------------------
;; F3: code-agent-chat--make-backend
;; ---------------------------------------------------------------------------
...
(ert-deftest test-chat-f3-make-backend-claude-code ()
  ...
  (let ((code-agent-chat-backend-type 'code-agent))
    (let ((backend (code-agent-chat--make-backend)))
...
(ert-deftest test-chat-f4-send-input-creates-backend-on-first-call ()
  ...
  (with-temp-buffer
    (let ((code-agent-chat--backend nil)
          (code-agent-chat--query-handle nil)
          (code-agent-chat--waiting nil)
          (code-agent-chat--current-response "")
          (code-agent-chat--session-id nil)
          (code-agent-chat--system-prompt nil)
          (code-agent-chat--mcp-config nil)
          (code-agent-chat--options nil)
          (code-agent-chat--response-start-marker nil)
          (code-agent-chat-backend-type 'code-agent)
          (code-agent-chat-include-ide-context nil)
          (query-called nil))
      (cl-letf (((symbol-function 'code-agent-backend-query)
                ...
                ((symbol-function 'code-agent-chat--insert)
                 (lambda (&rest _) nil))
                ((symbol-function 'code-agent-chat--show-prompt)
                 (lambda () nil)))
        (code-agent-chat--send-input "hello")
        (should code-agent-chat--backend)
        (should (code-agent-claude-code-backend-p code-agent-chat--backend))
        (should query-called)))))
...
(ert-deftest test-chat-f4-send-input-reuses-existing-backend ()
  ...
  :tags '(:unit :fast :stable :isolated :chat-backend :f4)
  (with-temp-buffer
    (let* ((mock-backend (code-agent-claude-code-backend--create :session-key "test"))
           (code-agent-chat--backend mock-backend)
           (code-agent-chat--query-handle nil)
           (code-agent-chat--waiting nil)
           (code-agent-chat--current-response "")
           (code-agent-chat--session-id nil)
    ...
    (let ((code-agent-chat--backend nil)
      ...
      (cl-letf (((symbol-function 'code-agent-backend-query)
                ...
                ((symbol-function 'code-agent-chat--show-prompt)
                 (lambda () nil)))
        (code-agent-chat--send-input "hello")
        (should (eq 'the-handle code-agent-chat--query-handle))))))
...
(ert-deftest test-chat-f4-send-input-passes-options ()
  ...
  (with-temp-buffer
    (let ((code-agent-chat--backend nil)
          ...
          (code-agent-chat--waiting nil)
          (code-agent-chat--current-response "")
          (code-agent-chat--session-id "resume-123")
          (code-agent-chat--system-prompt nil)
          (code-agent-chat--mcp-config nil)
          (code-agent-chat--options nil)
          (code-agent-chat--response-start-marker nil)
...
(ert-deftest test-chat-f5-interrupt-calls-backend-cancel ()
  ...
  (with-temp-buffer
    (let* ((cancel-called nil)
           ...
           (code-agent-chat--backend mock-backend)
           (code-agent-chat--query-handle 'the-handle)
           (code-agent-chat--waiting t))
...
(ert-deftest test-chat-f5-interrupt-no-backend-no-error ()
  ...
  (with-temp-buffer
    (let ((code-agent-chat--backend nil)
          (code-agent-chat--query-handle nil)
          (code-agent-chat--waiting t))
      (cl-letf (((symbol-function 'code-agent-chat--insert)
                 (lambda (&rest _) nil))
                ((symbol-function 'code-agent-chat--show-prompt)
                 (lambda () nil)))
        ;; Should not error
...
(ert-deftest test-chat-f5-interrupt-nils-handle ()
  ...
  :tags '(:unit :fast :stable :isolated :chat-backend :f5)
  (with-temp-buffer
    (let ((code-agent-chat--backend (code-agent-claude-code-backend--create))
          (code-agent-chat--query-handle 'some-handle)
          (code-agent-chat--waiting t))
      (cl-letf (((symbol-function 'code-agent-backend-cancel)
                 (lambda (&rest _) nil))
                ((symbol-function 'code-agent-chat--insert)
                ...
                ((symbol-function 'code-agent-chat--show-prompt)
                 (lambda () nil)))
        (code-agent-chat-interrupt)
        (should-not code-agent-chat--query-handle)))))
    ...
    (cl-letf (((symbol-function 'code-agent-backend-cleanup)
      ...
      (let ((buf (generate-new-buffer " *test-chat-kill*")))
        (with-current-buffer buf
          (setq-local code-agent-chat--backend
...
(ert-deftest test-chat-f6-kill-buffer-no-backend-no-error ()
  ...
  (let ((buf (generate-new-buffer " *test-chat-kill-nil*")))
    (with-current-buffer buf
      (setq-local code-agent-chat--backend nil)
      (add-hook 'kill-buffer-hook #'code-agent-chat--kill-buffer-cleanup nil t))
    ;; Should not error
    (kill-buffer buf)))
...
(ert-deftest test-chat-f7-clear-cleans-up-backend ()
  ...
  (with-temp-buffer
    (let ((cleanup-called nil)
          (code-agent-chat--backend (code-agent-claude-code-backend--create))
          (code-agent-chat--query-handle 'some-handle)
          (code-agent-chat--session-id "sess-1")
          (code-agent-chat--waiting nil)
          (code-agent-chat--total-cost 1.5)
          (code-agent-chat--current-response ""))
      (cl-letf (((symbol-function 'code-agent-backend-cleanup)
                 (lambda (backend) (setq cleanup-called t)))
                ((symbol-function 'yes-or-no-p)
                 (lambda (&rest _) t))
                ((symbol-function 'code-agent-chat--initialize-buffer)
                 (lambda () nil)))
        (let ((inhibit-read-only t))
          (code-agent-chat-clear))
        (should cleanup-called)
        (should-not code-agent-chat--backend)
        (should-not code-agent-chat--query-handle)
        (should-not code-agent-chat--session-id)))))
...
(ert-deftest test-chat-f7-new-session-claude-code-nils-session ()
  ...
  (with-temp-buffer
    (let ((code-agent-chat--backend (code-agent-claude-code-backend--create))
          (code-agent-chat--session-id "old-sess")
          (code-agent-chat-backend-type 'code-agent))
      (cl-letf (((symbol-function 'code-agent-chat--insert)
                 (lambda (&rest _) nil)))
...
(provide 'test-code-agent-chat-backend)
;;; test-code-agent-chat-backend.el ends here
