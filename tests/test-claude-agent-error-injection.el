;;; test-claude-agent-error-injection.el --- Error injection tests -*- lexical-binding: t; -*-
...
We manually kill the process after a short wait and verify cleanup."
  ...
  (let ((tokens '())
    ...
    (should (process-live-p (claude-agent--process-state-process state)))
...
(provide 'test-claude-agent-error-injection)
;;; test-claude-agent-error-injection.el ends here
