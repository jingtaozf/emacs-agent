;;; test-claude-org-telemetry.el --- Tests for F5: Basic telemetry -*- lexical-binding: t; -*-
...
FIX: In the completion handler, call org-entry-put for token counts."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "test-session"))
      ...
      (claude-org--session-put session-key :query-id "q-tel-1")
      ;; Simulate persisting usage data
      (claude-org--persist-telemetry session-key
                                     '(:input-tokens 1500
                                       :output-tokens 300
...
FIX: Record start-time on query begin, compute duration on completion."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "test-session"))
      ...
      (insert "Response text\n")
      (claude-org--session-put session-key :marker (point-min-marker))
      (claude-org--session-put session-key :query-id "q-tel-2")
      (claude-org--session-put session-key :start-time (- (float-time) 5.3))
      ;; Persist duration
...
FIX: Increment counter on each tool-use message, persist on completion."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "test-session"))
      ...
      (insert "Response text\n")
      (claude-org--session-put session-key :marker (point-min-marker))
      (claude-org--session-put session-key :query-id "q-tel-3")
      ;; Simulate 5 tool calls
      (claude-org--session-put session-key :tool-call-count 5)
      ;; Persist tool count
...
(provide 'test-claude-org-telemetry)
;;; test-claude-org-telemetry.el ends here
