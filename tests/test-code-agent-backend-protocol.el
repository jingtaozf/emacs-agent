;;; test-code-agent-backend-protocol.el --- F1: Protocol Extension Tests -*- lexical-binding: t; -*-
...
(ert-deftest test-f1-verbose-buffer-generic-defined ()
  ...
  (should (fboundp 'code-agent-backend-verbose-buffer)))

(ert-deftest test-f1-ready-p-generic-defined ()
  "backend-ready-p generic should be defined."
  :tags '(:unit :fast :stable :isolated :backend :f1)
  (should (fboundp 'code-agent-backend-ready-p)))
...
(ert-deftest test-f1-claude-code-backend-ready-p-active-query ()
  ...
  (let ((backend (code-agent-claude-code-backend--create)))
    ...
    (let ((mock-state (code-agent--make-process-state :request-id "test")))
...
(ert-deftest test-f1-claude-code-backend-hook-permissions-nil ()
  ...
  :tags '(:unit :fast :stable :isolated :backend :f1)
  (let ((backend (code-agent-claude-code-backend--create)))
    (should-not (code-agent-backend-supports-p backend :hook-permissions))))

(ert-deftest test-f1-claude-code-backend-terminal-verbose-nil ()
  "JSON backend should return nil for :terminal-verbose."
  :tags '(:unit :fast :stable :isolated :backend :f1)
  (let ((backend (code-agent-claude-code-backend--create)))
    (should-not (code-agent-backend-supports-p backend :terminal-verbose))))
...
(ert-deftest test-f1-existing-generics-still-work ()
  ...
  (let ((backend (code-agent-claude-code-backend--create)))
    ...
    (should (eq 'unknown (code-agent-backend-classify-error backend "foo")))
    ...
    (code-agent-backend-send-input backend "test")
    ;; supports-p should work
    (should (code-agent-backend-supports-p backend :streaming-tokens))))

(provide 'test-code-agent-backend-protocol)
;;; test-code-agent-backend-protocol.el ends here
