...
;;; F35b: No dangling calls (project-prefixed call sites must resolve to a defun)

;; 2026-07-20 incident: commit 26df975 (pivot to org-as-control-plane)
;; deleted `code-agent-org-cmux--wait-for-ready' but left the call in
;; `code-agent-org-cmux--launch-workspace' → void-function error at
;; runtime.  F35 above missed it because it only scans repo-root .org
;; files; the actual sources live under lp/org/.  This check walks
;; every source .org recursively and asserts every project-prefixed
;; call site has a matching definition.

(defvar test-structural--known-dangling-allowlist
  '()
  "Project-prefixed function names that are intentionally not defined
via defun/cl-defgeneric in any source .org file.  Add here only when
the symbol is resolved by an external mechanism (defstruct-generated
accessor, autoload from a third-party package, etc.).  Document the
reason next to each entry.")

(defun test-structural--all-elisp-org-source-files ()
  "All source .org files under the project root, recursively.
Excludes reference/, docs/, tests/, tasks/, .cache/ — same filter
as `test-structural--scan-semantic-duplicates'."
  (cl-remove-if
   (lambda (f)
   ...
   (directory-files-recursively test-structural--project-root "\\.org$")))

(defun test-structural--defined-function-set ()
  "Names defined via defun/cl-defun/defmacro/cl-defgeneric/cl-defmethod
in any source .org file, plus defalias/fset aliases, plus accessors and
predicates auto-generated by cl-defstruct, plus mode commands defined
by define-minor-mode / define-derived-mode.  Returns a hash-table
keyed by symbol name string."
  (let ((set (make-hash-table :test 'equal))
        (def-forms '(defun cl-defun defmacro cl-defgeneric cl-defmethod
                      defsubst cl-defsubst))
        (mode-forms '(define-minor-mode define-derived-mode
                       define-global-minor-mode define-globalized-minor-mode))
        (alias-forms '(defalias fset)))
    (dolist (file (test-structural--all-elisp-org-source-files))
      (dolist (sexp (test-structural--read-sexps-from-org-elisp-blocks file))
        (let ((head (car-safe sexp)))
          (cond
           ((memq head def-forms)
            (let ((name (cadr sexp)))
              (when (symbolp name)
                (puthash (symbol-name name) t set))))
           ((memq head mode-forms)
            (let ((name (cadr sexp)))
              (when (symbolp name)
                (puthash (symbol-name name) t set))))
           ((eq head 'cl-defstruct)
            ;; (cl-defstruct NAME SLOT...) or (cl-defstruct (NAME OPT...) SLOT...)
            ;; Each generates: make-NAME (unless :constructor overrides),
            ;; NAME-p (unless :predicate nil), NAME-SLOT per slot.
            ;; Custom names come from the (:constructor SYMBOL ...) and
            ;; (:predicate SYMBOL) options.
            (let* ((name-spec (cadr sexp))
              ...
              ;; Parse options for custom constructor / predicate names.
              (dolist (opt options)
                (when (and (consp opt)
                           (memq (car opt) '(:constructor :predicate :copier)))
                  (let ((kind (car opt))
                        (arg (cadr opt)))
                    (cond
                     ((and (eq kind :constructor) (symbolp arg))
                      (push arg custom-ctors))
                     ((and (eq kind :predicate) (symbolp arg))
                      (setq custom-pred arg))
                     ((and (eq kind :constructor) (null arg))
                      (setq default-ctor-suppressed t))))))
              (when (symbolp struct-name)
                (let ((nm (symbol-name struct-name)))
                  (unless (or custom-ctors default-ctor-suppressed)
                    (puthash (concat "make-" nm) t set))
                  (dolist (ctor custom-ctors)
                    (puthash (symbol-name ctor) t set))
                  (if custom-pred
                      (puthash (symbol-name custom-pred) t set)
                    (puthash (concat nm "-p") t set))
                  (dolist (slot slots)
                    (let ((slot-name
                           (if (symbolp slot)
                               slot
                      ...
                      (when (symbolp slot-name)
                        (puthash (concat nm "-"
                                 ...
                                 t set))))))))
           ((memq head alias-forms)
            ;; (defalias 'foo ...) or (fset 'foo ...) — first arg is
            ;; usually a quoted symbol.
            (let ((arg (cadr sexp)))
              (when (eq (car-safe arg) 'quote)
                (let ((name (cadr arg)))
                  (when (symbolp name)
                    (puthash (symbol-name name) t set))))))))))
    ;; code-agent.el entry point
    (let ((entry (expand-file-name "code-agent.el" test-structural--project-root)))
      (when (file-exists-p entry)
        (with-temp-buffer
          (insert-file-contents entry)
          (goto-char (point-min))
          (while (re-search-forward
                  "^(\\(?:defun\\|cl-defun\\|defmacro\\|cl-defgeneric\\|cl-defmethod\\)\\s-+\\(\\S-+\\)"
                  nil t)
            (puthash (match-string-no-properties 1) t set)))))
    set))

(defun test-structural--walk-project-calls (sexp prefix-re file calls)
  "Recursively walk SEXP; record project-prefixed call sites into CALLS.
CALLS is a hash-table name→list-of-files.  Skips (quote ...) subtrees
so quoted data is not mistaken for a call.  For special forms whose
cadr is data rather than a call position (defvar/defcustom/defconst/
setq/let/let*/lambda), the data position is not walked — otherwise a
variable name like `code-agent-trace--current-context' would be
misreported as a call.  Children are walked via an explicit cons-cell
loop (not `dolist') so dotted-pair literal data does not raise
wrong-type-argument."
  (when (consp sexp)
    (let ((head (car sexp)))
      (unless (eq head 'quote)
        (when (and (symbolp head)
                   (string-match-p prefix-re (symbol-name head)))
          (push file (gethash (symbol-name head) calls nil)))
        ;; Determine which children to recurse into, based on the form head.
        (cond
         ;; Definitions — skip the metadata, walk only the body.
         ;; (defun/cl-defun/defmacro NAME ARGS [DOC] [DECL] BODY...)
         ;; (cl-defmethod NAME ARGS BODY...)
         ((memq head '(defun cl-defun defmacro cl-defmethod))
          (let ((tail (cdddr sexp)))
            (while (consp tail)
              (test-structural--walk-project-calls
               (car tail) prefix-re file calls)
              (setq tail (cdr tail)))))
         ;; cl-defstruct — options and slot specs are not call sites.
         ((eq head 'cl-defstruct) nil)
         ;; cl-defgeneric — declarations only, no body to walk.
         ((eq head 'cl-defgeneric) nil)
         ;; (defvar/defcustom/defconst X VAL [DOC]) — skip X, walk VAL.
         ((memq head '(defvar defcustom defconst defparameter
                        define-symbol-macro))
          (let ((tail (cddr sexp)))
            (while (consp tail)
              (test-structural--walk-project-calls
               (car tail) prefix-re file calls)
              (setq tail (cdr tail)))))
         ;; (let/let* BINDINGS BODY...) and (lambda ARGLIST BODY...)
         ;; — skip cadr (the binding/arglist), walk the body.
         ((memq head '(let let* lambda))
          (let ((tail (cddr sexp)))
            (while (consp tail)
              (test-structural--walk-project-calls
               (car tail) prefix-re file calls)
              (setq tail (cdr tail)))))
         ;; (setq V1 E1 V2 E2 ...) — walk even-position args (E),
         ;; skip odd-position (V).
         ((eq head 'setq)
          (let ((tail (cdr sexp)) (pos 0))
            (while (consp tail)
              (when (eq pos 1)
                (test-structural--walk-project-calls
                 (car tail) prefix-re file calls))
              (setq pos (if (eq pos 1) 0 1)
                    tail (cdr tail)))))
...
(ert-deftest test-structural-no-dangling-calls ()
  "Every project-prefixed function call in source .org must resolve to a
definition.  Catches dangling references left behind when a refactor
deletes a defun but misses a call site.
FIX: Restore the deleted defun (check git log -S), or remove the call
site, or — if the symbol is resolved externally — add it to
`test-structural--known-dangling-allowlist' with a reason."
  :tags '(:unit :fast :stable :structural)
  (when test-structural--project-root
    (let* ((defined (test-structural--defined-function-set))
           (prefix-re "^\\(code-agent-\\|claude-\\|emacs-mcp-server-\\)")
           (calls (make-hash-table :test 'equal)))
      (dolist (file (test-structural--all-elisp-org-source-files))
        (dolist (sexp (test-structural--read-sexps-from-org-elisp-blocks file))
          (test-structural--walk-project-calls sexp prefix-re file calls)))
      (let ((dangling nil))
        (maphash
         (lambda (name files)
           (unless (or (gethash name defined)
                       (member name test-structural--known-dangling-allowlist))
             (push (cons name (car files)) dangling)))
         calls)
        (should-with-fix (null dangling)
          (format "Dangling calls (called but never defined):\n%s\nFIX: Restore the deleted defun (try `git log -S %s`), or remove the call, or add to test-structural--known-dangling-allowlist."
                  (mapconcat
                   (lambda (entry)
                     (format "  %s (in %s)"
                             (car entry)
                             (file-relative-name (cdr entry)
                                                 test-structural--project-root)))
                   (sort dangling (lambda (a b) (string< (car a) (car b))))
                   "\n")
                  (if dangling (caar dangling) "")))))))
