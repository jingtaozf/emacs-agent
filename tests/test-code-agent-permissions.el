;;; test-code-agent-permissions.el --- Tests for Permission Functions System -*- lexical-binding: t; -*-
...
;; Note: code-agent.org is loaded via Makefile before this file is loaded
...
(ert-deftest test-permission-check-patterns-allow ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions)
  (let ((code-agent-allow-patterns '("Read" "Grep" "Glob"))
        (code-agent-deny-patterns nil))
    (let ((result (code-agent-permission-check-patterns
                   "Read" '(:file_path "/tmp/test.txt") nil)))
      (should (equal (plist-get result :behavior) "allow")))))
...
(ert-deftest test-permission-check-patterns-deny ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions)
  (let ((code-agent-allow-patterns nil)
        (code-agent-deny-patterns '("Write" "Edit")))
    (let ((result (code-agent-permission-check-patterns
                   "Write" '(:file_path "/tmp/test.txt") nil)))
      (should (equal (plist-get result :behavior) "deny")))))
...
(ert-deftest test-permission-check-patterns-passthrough ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions)
  (let ((code-agent-allow-patterns '("Read"))
        (code-agent-deny-patterns nil))
    (let ((result (code-agent-permission-check-patterns
                   "Write" '(:file_path "/tmp/test.txt") nil)))
...
(ert-deftest test-permission-auto-allow ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions)
  (let ((result (code-agent-permission-auto-allow
...
(ert-deftest test-format-question-options-empty ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions :ask-user)
  (let ((formatted (code-agent--format-question-options nil)))
  ...
  (cl-letf (((symbol-function 'completing-read)
    ...
    (let* ((tool-input (list :questions
                             (list (list :question "How should I format?"
                                        ...
                                        :multiSelect nil))))
           (result (code-agent-permission-ask-user-question
...
(ert-deftest test-ask-user-question-mock-answers ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions :ask-user)
  (let ((code-agent-ask-user-question-mock-answers
...
(ert-deftest test-permission-function-error-logged ()
  ...
  (let ((logged-errors nil)
    ...
    (cl-letf (((symbol-function 'message)
      ...
      (code-agent--run-permission-functions "Read" nil nil))
...
(ert-deftest test-permission-error-includes-function-info ()
  ...
  :tags '(:unit :fast :stable :isolated :permissions :tdd)
  (let ((code-agent-permission-functions
         (list #'code-agent-permission-check-patterns  ; this won't error
               (lambda (_t _i _c)
                 (error "Intentional test error")))))
    ;; Temporarily make check-patterns error
    (cl-letf (((symbol-function 'code-agent-permission-check-patterns)
...
(provide 'test-code-agent-permissions)
;;; test-code-agent-permissions.el ends here
