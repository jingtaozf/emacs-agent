;;; test-claude-org-cleanup.el --- Tests for centralized session cleanup  -*- lexical-binding: t; -*-
...
(ert-deftest test-cleanup-session-sets-terminal-state ()
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((sk "test-session"))
      ...
      ;; Pre-conditions
      (should (claude-org--session-get sk :busy))
      (should (claude-org--session-get sk :recovering))
      (should (claude-org--session-get sk :marker))
      ;; Act
      (claude-org--cleanup-session sk "completed")
      ;; Assert
      (should-not (claude-org--session-get sk :busy))
      (should-not (claude-org--session-get sk :recovering))
      (should-not (claude-org--session-get sk :marker)))))
...
(ert-deftest test-cleanup-session-clears-queue-when-requested ()
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((sk "test-session"))
      ...
      (should (claude-org--session-get sk :pending-queue))
      ;; Act: cleanup WITH clear-queue
      (claude-org--cleanup-session sk "error" t)
...
BUG A2: insert-error currently inserts at marker (inside/near src block)."
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((sk "test-err-session"))
      ...
      ;; Act
      (claude-org--insert-error sk "Something went wrong")
...
BUG B3: Currently uses point-min without widen, silently fails."
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((result (claude-org--find-response-by-query-id "qid-narrow-test")))
...
Depends on find-response-by-query-id widening."
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((sk "test-narrow"))
      ...
      (let ((result (claude-org--insert-at-response sk "Hello from Claude")))
        (should result))
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((sk "test-nil-qid")
      ...
      (cl-letf (((symbol-function 'message)
        ...
        (claude-org--handle-token-v2 sk "Hello world"))
...
BUG: loop-abort currently only does 2/8 cleanup operations."
  ...
  (test-cleanup--with-org-buffer
    ...
    (let ((sk "test-loop-abort")
      ...
      (claude-org--cleanup-session sk "error")
...
(ert-deftest test-cleanup-queued-blocks-frees-markers ()
  ...
  (test-cleanup--with-org-buffer
    ...
    (let* ((sk "test-queue-cleanup")
      ...
      ;; Act
      (claude-org--cleanup-queued-blocks sk)
      ;; Queued marker should be freed
      (should-not (marker-position q-marker))
      ;; Queue should be cleared
      (should-not (claude-org--session-get sk :pending-queue)))))

(provide 'test-claude-org-cleanup)
;;; test-claude-org-cleanup.el ends here
