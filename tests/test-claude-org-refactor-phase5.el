;;; test-claude-org-refactor-phase5.el --- Phase 5 test infrastructure tests -*- lexical-binding: t; -*-
...
(ert-deftest test-f15-with-session-creates-state ()
  ...
  (test-org-with-session "* Test\n" "test::f15"
    ...
    (should (claude-org--session-state-p
             (gethash "test::f15" claude-org--sessions)))))
...
(ert-deftest test-f15-session-assertion-helpers ()
  ...
  :tags '(:unit :fast :stable :isolated :refactor :f15)
  (test-org-with-session "* Test\n" "test::f15-assert"
    (claude-org--session-put "test::f15-assert" :busy t)
    (test-org-session-field-eq "test::f15-assert" :busy t)
    (claude-org--session-put "test::f15-assert" :busy nil)
...
(ert-deftest test-f15-fixtures-used-in-phase4 ()
  ...
  (test-org-with-sessions ""
    (should (hash-table-p claude-org--sessions))
    (claude-org--session-put "test::meta" :busy t)
    (should (eq t (claude-org--session-get "test::meta" :busy)))))

(provide 'test-claude-org-refactor-phase5)
;;; test-claude-org-refactor-phase5.el ends here
