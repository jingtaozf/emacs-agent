...
(ert-deftest test-claude-org-backtrace-text-with-buffer ()
  ...
  (let ((buf (get-buffer-create "*Backtrace*")))
    (unwind-protect
        (progn
          ...
          (let ((result (claude-org-template--backtrace-text)))
            (should (string-match-p "void-variable foo" result))))
      (kill-buffer buf))))

(ert-deftest test-claude-org-backtrace-text-no-buffer ()
...
(ert-deftest test-claude-org-yasnippet-snippets-loaded ()
...
(ert-deftest test-claude-org-session-state-struct-exists ()
  ...
  (let ((ss (claude-org--make-session-state)))
    ...
    (should-not (claude-org--session-state-get ss :busy))
    (should-not (claude-org--session-state-get ss :recovering))
    (should-not (claude-org--session-state-get ss :query-id))
    (should-not (claude-org--session-state-get ss :backend))
    (should-not (claude-org--session-state-get ss :query-handle))
    (should-not (claude-org--session-state-get ss :marker))
    (should-not (claude-org--session-state-get ss :custom-id))
    (should-not (claude-org--session-state-get ss :block-id))
    (should-not (claude-org--session-state-get ss :pending-queue))))
...
(ert-deftest test-claude-org-session-state-struct-fields ()
  ...
  (let ((ss (claude-org--make-session-state
             ...
             :spinner 2
             :sdk-uuid "uuid-xyz")))
    (should (eq t (claude-org--session-state-get ss :busy)))
    (should (equal "qid-123" (claude-org--session-state-get ss :query-id)))
    (should (equal 'mock-backend (claude-org--session-state-get ss :backend)))
    (should (equal 'mock-handle (claude-org--session-state-get ss :query-handle)))
    (should (= 1234.5 (claude-org--session-state-get ss :start-time)))
    (should (equal "hello" (claude-org--session-state-get ss :original-prompt)))
    (should (= 3 (claude-org--session-state-get ss :section-level)))
    (should (eq t (claude-org--session-state-get ss :response-has-content)))
    (should (equal "qid-prev" (claude-org--session-state-get ss :last-assistant-query-id)))
    (should (= 42 (claude-org--session-state-get ss :current-line-length)))
    (should (= 5 (claude-org--session-state-get ss :loop-max)))
    (should (= 2 (claude-org--session-state-get ss :loop-current)))
    (should (= 10 (claude-org--session-state-get ss :loop-interval)))
    (should (equal '("block-1") (claude-org--session-state-get ss :pending-queue)))
    (should (= 3 (claude-org--session-state-get ss :instruction-num)))
    (should (equal "cid-abc" (claude-org--session-state-get ss :custom-id)))
    (should (= 1 (claude-org--session-state-get ss :recovery-count)))
    (should (equal "bid-001" (claude-org--session-state-get ss :block-id)))
    (should (= 2 (claude-org--session-state-get ss :spinner)))
    (should (equal "uuid-xyz" (claude-org--session-state-get ss :sdk-uuid)))))

(ert-deftest test-claude-org-session-state-setf ()
  "Test that session-state fields are mutable via session-state-set."
  :tags '(:unit :fast :stable :isolated :session :phase-7)
  (let ((ss (claude-org--make-session-state)))
    (claude-org--session-state-set ss :busy t)
    (claude-org--session-state-set ss :query-id "qid-new")
    (should (eq t (claude-org--session-state-get ss :busy)))
    (should (equal "qid-new" (claude-org--session-state-get ss :query-id)))))
...
(ert-deftest test-f9-show-verbose-uses-backend-verbose-buffer ()
  ...
  (let* ((test-buf (generate-new-buffer " *test-verbose*"))
         (backend (claude-agent-json-backend--create)))
    (unwind-protect
        (progn
          ;; backend-verbose-buffer returns nil for json-backend (no terminal)
...
(ert-deftest test-f9b-backend-property-file-level-json-stream ()
  "File-level CLAUDE_BACKEND property json-stream creates json-backend."
