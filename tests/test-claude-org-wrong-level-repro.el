;;; test-claude-org-wrong-level-repro.el --- Tests reproducing wrong section level bugs -*- lexical-binding: t; -*-
...
;;
;; Key bug: In `claude-org--create-response-section' (claude-org.org:1893):
...
;;
;; Similarly in `claude-org--handle-token-v2' (line 2015):
...
BUG: Line 1893 of claude-org.org:
  (let* ((section-level (or (claude-org--session-get session-key :section-level) 1))
...
Instead it silently produces an invalid org heading."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "/tmp/test-level-zero.org"))
      ...
      (should (null (claude-org--session-get session-key :section-level)))
      ;; Set it explicitly to 0 (simulating AI block before first heading)
      (claude-org--session-put session-key :section-level 0)
      (claude-org--session-put session-key :query-id "test-qid-001")
      (goto-char (point-max))
      ;; Call create-response-section
      (claude-org--create-response-section session-key "test-qid-001" 'normal)
...
claude-org-execute stores the correct level, the heading has no stars.
...
the execute function has written the correct section-level to session state."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "/tmp/test-level-fallback.org"))
      ...
      (claude-org--session-put session-key :query-id "test-qid-002")
      (goto-char (point-max))
      (claude-org--create-response-section session-key "test-qid-002" 'normal)
...
the response heading should use that level."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "/tmp/test-level-queued.org"))
      ...
      (claude-org--session-put session-key :query-id "block-a-qid")
      ...
      (claude-org--session-put session-key :section-level 5)
      (claude-org--session-put session-key :query-id "block-b-qid")
      ;; Create Block B's response section
      (goto-char (point-max))
      (claude-org--create-response-section session-key "block-b-qid" 'normal)
...
heading's level, not the instruction heading's level."
  ...
  (with-temp-buffer
    ...
    (let ((level-inside-response (claude-org--get-section-level)))
...
But because section-level was 0 (never properly set), we get level 1."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "/tmp/test-normalize-level.org"))
      ...
      (let* ((target-level (1+ (let ((raw (claude-org--session-get session-key :section-level)))
        ...
        (let ((claude-org-normalize-headers t)  ; ensure normalization is on
              (result (claude-org--normalize-headers-in-text "* My Heading\nContent\n" target-level)))
...
sequencing guarantee ever breaks."
  ...
  (with-temp-buffer
    ...
    (let ((session-key "/tmp/test-overwrite-level.org"))
      ...
      (claude-org--session-put session-key :section-level 2)
      (claude-org--session-put session-key :query-id "block-a-qid")
      ;; Verify Block A's target-level for header normalization
      (let ((target-level-a (1+ (or (claude-org--session-get session-key :section-level) 0))))
      ...
      (let ((target-level-after-overwrite
             (1+ (or (claude-org--session-get session-key :section-level) 0))))
...
instruction heading's level (e.g., 2) to find sibling boundaries correctly."
  ...
  (with-temp-buffer
    ...
    (org-back-to-heading t)
    (let ((boundary-from-instruction (claude-org--find-response-boundary)))
      ...
      (org-back-to-heading t)
      (let ((boundary-from-nested (claude-org--find-response-boundary)))
...
BUG: Line 3741: (let* ((level (or section-level (claude-org--get-section-level)))
...
the function returns the wrong level."
  ...
  (with-temp-buffer
    ...
    ;; It will call (claude-org--get-section-level) at point
    (let ((header (claude-org--create-response-section-header
...
(provide 'test-claude-org-wrong-level-repro)
;;; test-claude-org-wrong-level-repro.el ends here
