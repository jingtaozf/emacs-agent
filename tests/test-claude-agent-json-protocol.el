;;; test-claude-agent-json-protocol.el --- TDD Tests for JSON Protocol Handling -*- lexical-binding: t; -*-
...
(ert-deftest test-json-validate-message-valid-types ()
  ...
  (let ((msg '(:type "assistant" :message (:content "hello"))))
    (should (claude-agent--validate-json-message msg)))
  ;; user message
...
(ert-deftest test-json-validate-message-missing-type ()
  ...
  (let ((msg '(:message "hello" :content "test")))
    (should-not (claude-agent--validate-json-message msg))))
...
(ert-deftest test-json-parse-error-is-logged ()
  ...
  (let ((logged-errors nil))
    (cl-letf (((symbol-function 'claude-agent--debug-log)
...
actually malformed JSON that was supposed to be valid."
  ...
  (should (claude-agent--is-cli-text-output-p "Welcome to Claude CLI"))
  (should (claude-agent--is-cli-text-output-p "Error: Connection failed"))
  (should (claude-agent--is-cli-text-output-p ""))
  ;; Clearly attempted JSON (has braces)
  (should-not (claude-agent--is-cli-text-output-p "{broken"))
  (should-not (claude-agent--is-cli-text-output-p "{\"type\":"))
  (should-not (claude-agent--is-cli-text-output-p "[1, 2, 3")))
...
(provide 'test-claude-agent-json-protocol)
;;; test-claude-agent-json-protocol.el ends here
