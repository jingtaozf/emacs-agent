;;; test-code-agent-state-management.el --- Tests for query lifecycle state management -*- lexical-binding: t -*-
...
;;   2. registry-cleanup-process removes from code-agent--registry
;;   3. BUT it does NOT call code-agent--unregister-query
;;   4. The old dead entry remains in code-agent--active-queries
...
removes from the registry but NOT from code-agent--active-queries.
The old dead entry persists in the queries buffer."
  ...
  (let* ((proc (start-process "test-abnormal-exit" nil "sleep" "10"))
         ...
         (saved-active-queries (copy-hash-table code-agent--active-queries))
         (saved-active-states (copy-sequence (code-agent-registry-active-states))))
    (unwind-protect
        (progn
          ;; Register the query in active-queries
          (code-agent--register-query "test-abnormal-001" state)
          (should (gethash "test-abnormal-001" code-agent--active-queries))
          ...
          ;; only removes from code-agent--registry, not code-agent--active-queries
          (should-not (gethash "test-abnormal-001" code-agent--active-queries)))
      ;; Cleanup
      (when (process-live-p proc) (delete-process proc))
      (setq code-agent--active-queries saved-active-queries)
      (setf (code-agent--registry-active-states code-agent--registry) saved-active-states))))
...
This test verifies the normal path works correctly as a control case."
  ...
  (let* ((proc (start-process "test-normal-exit" nil "sleep" "10"))
    ...
    (unwind-protect
        (progn
          ;; Register the query
          (code-agent--register-query "test-normal-001" state)
          (setf (code-agent--registry-active-states code-agent--registry)
                    (cons state (code-agent-registry-active-states)))
          (should (gethash "test-normal-001" code-agent--active-queries))
          ...
          (should-not (gethash "test-normal-001" code-agent--active-queries)))
      ;; Cleanup
      (when (process-live-p proc) (delete-process proc))
      (setq code-agent--active-queries saved-active-queries)
      (setf (code-agent--registry-active-states code-agent--registry) saved-active-states))))
...
Bug: When a process is killed by signal 9:
  ...
  (let* ((proc (start-process "test-oom-kill" nil "sleep" "10"))
         (state (code-agent--make-process-state
                 ...
                 :session-key "test-session-key"))
         (code-agent-auto-recovery t)
         (saved-active-queries (copy-hash-table code-agent--active-queries))
         (saved-active-states (copy-sequence (code-agent-registry-active-states))))
    (unwind-protect
        (progn
          ;; Register the query
          (code-agent--register-query "test-oom-001" state)
          (should (gethash "test-oom-001" code-agent--active-queries))
          ...
          ;; doesn't touch code-agent--active-queries
          (should-not (gethash "test-oom-001" code-agent--active-queries)))
      ;; Cleanup
      (when (process-live-p proc) (delete-process proc))
      (setq code-agent--active-queries saved-active-queries)
      (setf (code-agent--registry-active-states code-agent--registry) saved-active-states))))
...
as the normal exit path regarding active-queries hash table."
  ...
  (let* ((saved-active-queries (copy-hash-table code-agent--active-queries))
    ...
    (unwind-protect
        (progn
          ;; Register both
          (code-agent--register-query "test-parity-normal-001" state-normal)
          (code-agent--register-query "test-parity-abnormal-001" state-abnormal)
          (setf (code-agent--registry-active-states code-agent--registry)
                (cons state-normal (code-agent-registry-active-states)))
          (setf (code-agent--registry-active-states code-agent--registry)
                (cons state-abnormal (code-agent-registry-active-states)))

          ;; Normal exit path
          (setf (code-agent--process-state-closed state-normal) t)
          (code-agent--sentinel-cleanup proc-normal state-normal)
          (should-not (gethash "test-parity-normal-001" code-agent--active-queries))

          ;; Abnormal exit path (recovery cleanup only, no resume-session)
          (setf (code-agent--process-state-closed state-abnormal) t)
          (code-agent-registry-cleanup-process state-abnormal)

          ;; BUG: abnormal exit should have same result as normal exit
          ;; regarding active-queries cleanup
          (should-not (gethash "test-parity-abnormal-001" code-agent--active-queries)))
      ;; Cleanup
  ...
  (let* ((proc (start-process "test-cancel-killed" nil "sleep" "10"))
    ...
    (unwind-protect
        (progn
          ...
          (code-agent--sentinel-handle-normal-exit state "killed: 2\n")
...
tracking systems, which is the root cause of the cleanup inconsistency."
  :tags '(:unit :fast :stable :isolated :state-management)
  (let ((registry-queries (code-agent--registry-queries code-agent--registry)))
    ;; They should be different objects
    (should-not (eq registry-queries code-agent--active-queries))))
...
(ert-deftest test-register-query-only-adds-to-active-queries ()
  ...
  :tags '(:unit :fast :stable :isolated :state-management)
  (let* ((state (code-agent--make-process-state :request-id "test-dual-001"))
         (saved-active-queries (copy-hash-table code-agent--active-queries))
         (registry-queries (code-agent--registry-queries code-agent--registry)))
    (unwind-protect
        (progn
          (code-agent--register-query "test-dual-001" state)
          ;; Should be in active-queries
          (should (gethash "test-dual-001" code-agent--active-queries))
          ;; Should NOT be in registry-queries (separate system)
          (should-not (gethash "test-dual-001" registry-queries)))
      ;; Cleanup
      (remhash "test-dual-001" code-agent--active-queries)
      (setq code-agent--active-queries saved-active-queries))))

(provide 'test-code-agent-state-management)
;;; test-code-agent-state-management.el ends here
