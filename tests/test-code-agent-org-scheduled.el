;;; test-code-agent-org-scheduled.el --- Tests for scheduled AI block execution -*- lexical-binding: t; -*-
...
;; Unit tests for scheduled AI block execution in code-agent-org.
...
(ert-deftest test-scheduled-should-execute-never-run ()
  ...
  (let ((scheduled-time (time-subtract (current-time) (seconds-to-time 3600))))
    ...
    (should (code-agent-org-scheduled--should-execute-p scheduled-time ""))))
...
(ert-deftest test-scheduled-should-execute-already-done ()
  ...
  (let* ((scheduled-time (time-subtract (current-time) (seconds-to-time 3600)))
    ...
    (should-not (code-agent-org-scheduled--should-execute-p scheduled-time last-executed-str))))
...
(ert-deftest test-scheduled-should-execute-new-schedule ()
  ...
  (let* ((scheduled-time (time-subtract (current-time) (seconds-to-time 3600)))
    ...
    (should (code-agent-org-scheduled--should-execute-p scheduled-time last-executed-str))))
...
(ert-deftest test-scheduled-update-entry-new ()
  ...
  (let ((code-agent-org--scheduled-blocks nil)
    ...
    (code-agent-org-scheduled--update-entry "test-id" "/path/to/file.org" test-time)
    (should (= 1 (length code-agent-org--scheduled-blocks)))
    (should (equal "test-id" (caar code-agent-org--scheduled-blocks)))
    (should (equal "/path/to/file.org"
                   (plist-get (cdar code-agent-org--scheduled-blocks) :file)))))
...
(ert-deftest test-scheduled-update-entry-existing ()
  ...
  (let* ((old-time (time-subtract (current-time) (seconds-to-time 3600)))
    ...
    (code-agent-org-scheduled--update-entry "test-id" "/new/path.org" new-time)
    (should (= 1 (length code-agent-org--scheduled-blocks)))
...
(ert-deftest test-scheduled-remove-entry ()
  ...
  :tags '(:unit :fast :stable :isolated :scheduled)
  (let ((code-agent-org--scheduled-blocks
         (list (cons "id1" (list :file "/path1.org"))
               (cons "id2" (list :file "/path2.org")))))
    (code-agent-org-scheduled--remove-entry "id1")
    (should (= 1 (length code-agent-org--scheduled-blocks)))
    (should (equal "id2" (caar code-agent-org--scheduled-blocks)))))
...
"
   ...
   (should (code-agent-org-scheduled--has-ai-block-p))))
...
"
   ...
   (should-not (code-agent-org-scheduled--has-ai-block-p))))
...
Just text.
"))
          (let ((results (code-agent-org-scheduled--collect-from-file temp-file)))
...
"
   ...
   (should-not (code-agent-org-scheduled--advance-repeater))
...
(ert-deftest test-scheduled-mode-line-update-with-blocks ()
  ...
  (let ((code-agent-org--scheduled-timer (current-time))  ; Fake active timer
        (code-agent-org--scheduled-blocks
         (list (cons "id1" (list :file "/path1.org" :scheduled-time (current-time)))
               (cons "id2" (list :file "/path2.org" :scheduled-time (current-time)))))
        (code-agent-org-scheduled-string ""))
    (code-agent-org-scheduled--update-mode-line)
    (should (string-match-p "\\[S:2\\]" code-agent-org-scheduled-string))
    (should (get-text-property 0 'help-echo code-agent-org-scheduled-string))
    (should (get-text-property 0 'local-map code-agent-org-scheduled-string))))
...
(ert-deftest test-scheduled-mode-line-update-empty ()
  ...
  (let ((code-agent-org--scheduled-timer (current-time))  ; Fake active timer
        ...
        (code-agent-org-scheduled-string "previous value"))
    (code-agent-org-scheduled--update-mode-line)
    (should (string-empty-p code-agent-org-scheduled-string))))

(ert-deftest test-scheduled-mode-line-update-no-timer ()
  "Mode-line should be empty when scheduler not running."
  :tags '(:unit :fast :stable :isolated :scheduled :mode-line)
  (let ((code-agent-org--scheduled-timer nil)  ; No active timer
        (code-agent-org--scheduled-blocks
         (list (cons "id1" (list :file "/path1.org"))))
        (code-agent-org-scheduled-string "previous value"))
    (code-agent-org-scheduled--update-mode-line)
...
(ert-deftest test-scheduled-build-list-entries ()
  ...
  (let* ((test-time (time-subtract (current-time) (seconds-to-time 3600)))
    ...
    (let ((entries (code-agent-org-scheduled--build-list-entries)))
...
(ert-deftest test-scheduled-format-time ()
  ...
  :tags '(:unit :fast :stable :isolated :scheduled :list-buffer)
  ;; nil returns "Never"
  (should (equal "Never" (code-agent-org-scheduled--format-time nil)))
  ;; Non-nil returns formatted time
  (let ((result (code-agent-org-scheduled--format-time (current-time))))
...
"))
          ...
          (let ((code-agent-org--scheduled-blocks
            ...
            ;; Navigate
            (code-agent-org-scheduled--goto-block "target-block")
...
(ert-deftest test-scheduled-start-adds-mode-line ()
  ...
  (let ((code-agent-org--scheduled-timer nil)
        (code-agent-org--scheduled-blocks nil)
        (code-agent-org-scheduled-string "")
        (code-agent-org-scheduled-files nil)
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'org-agenda-files) (lambda (&rest _) nil)))
            ...
            ;; Timer should be set
            (should code-agent-org--scheduled-timer)
            ;; Mode-line should be in misc-info
            (should (member '(:eval code-agent-org-scheduled-string) mode-line-misc-info))))
      ;; Cleanup
...
(ert-deftest test-scheduled-stop-removes-mode-line ()
  ...
  :tags '(:unit :fast :stable :isolated :scheduled :integration)
  (let ((code-agent-org--scheduled-timer (run-at-time nil nil #'ignore))
        (code-agent-org-scheduled-string " [S:3]")
        (mode-line-misc-info (list '(:eval code-agent-org-scheduled-string) 'other)))
    (code-agent-org-scheduled-stop)
    ;; Timer should be nil
    (should-not code-agent-org--scheduled-timer)
    ;; Mode-line string should be empty
    (should (string-empty-p code-agent-org-scheduled-string))
    ;; Should be removed from misc-info
    (should-not (member '(:eval code-agent-org-scheduled-string) mode-line-misc-info))))

(provide 'test-code-agent-org-scheduled)
;;; test-code-agent-org-scheduled.el ends here
