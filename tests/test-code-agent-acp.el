;;; test-code-agent-acp.el --- Tests for ACP backend -*- lexical-binding: t; -*-

;;; Commentary:
;; Unit tests for code-agent-acp.org using our own JSON-RPC transport.
...
;; Load dependencies
(when (boundp 'test-code-agent-project-root)
  (let ((root test-code-agent-project-root))
    (unless (featurep 'code-agent-backend)
      (literate-elisp-load (expand-file-name "lp/sdk/code-agent-backend.org" root)))
    (unless (featurep 'code-agent-jsonrpc)
      (literate-elisp-load (expand-file-name "lp/backend/code-agent-jsonrpc.org" root)))
    (unless (featurep 'code-agent-acp)
      (literate-elisp-load (expand-file-name "lp/backend/code-agent-acp.org" root)))
    (unless (featurep 'code-agent-acp-opencode)
      (literate-elisp-load (expand-file-name "lp/backend/code-agent-acp-opencode.org" root)))
    (unless (featurep 'code-agent-acp-gemini)
      (literate-elisp-load (expand-file-name "lp/backend/code-agent-acp-gemini.org" root)))
    (unless (featurep 'code-agent-acp-codex)
      (literate-elisp-load (expand-file-name "lp/backend/code-agent-acp-codex.org" root)))))

(require 'code-agent-backend)
(require 'code-agent-jsonrpc)
(require 'code-agent-acp)
(require 'code-agent-acp-opencode)
(require 'code-agent-acp-gemini)
(require 'code-agent-acp-codex)
...
(ert-deftest test-acp-backend-struct-creation ()
  ...
  (let ((backend (code-agent-acp-backend--create
                  :session-key "test"
                  :cwd "/tmp")))
    (should (code-agent-acp-backend-p backend))
    (should (eq (code-agent-backend-type backend) :acp))
    (should (equal (code-agent-acp-backend-session-key backend) "test"))
...
(ert-deftest test-acp-opencode-constructor-sets-defaults ()
  ...
  (let ((backend (code-agent-acp-opencode-create
    ...
    (should (equal (code-agent-acp-backend-spawn-args backend)
                   '("opencode" "acp")))
    (should-not (code-agent-acp-backend-needs-authenticate backend))
    (should-not (code-agent-acp-backend-auth-request-maker backend))))

(ert-deftest test-acp-gemini-constructor-sets-defaults ()
  "Gemini constructor presets command, auth-method, and needs-auth."
  (let ((backend (code-agent-acp-gemini-create
                  :session-key "gm" :cwd "/tmp")))
    (should (equal (code-agent-acp-backend-agent-name backend) "gemini"))
    (should (equal (code-agent-acp-backend-spawn-args backend)
                   '("gemini" "--experimental-acp")))
    (should (code-agent-acp-backend-needs-authenticate backend))
    (should (functionp
             (code-agent-acp-backend-auth-request-maker backend)))))
...
shared `code-agent-acp--make-profile-backend' helper that installs
...
closure off a created backend instance."
  (dolist (method '(vertex-ai oauth-personal gemini-api-key))
    (let* ((code-agent-acp-gemini-auth-method method)
           (backend (code-agent-acp-gemini-create
                     :session-key "gm" :cwd "/tmp"))
           (maker (code-agent-acp-backend-auth-request-maker backend)))
...
(ert-deftest test-acp-codex-constructor-sets-defaults ()
  ...
  (let ((backend (code-agent-acp-codex-create
                  :session-key "cx" :cwd "/tmp")))
    (should (equal (code-agent-acp-backend-agent-name backend) "codex"))
    (should (equal (code-agent-acp-backend-spawn-args backend)
                   '("codex-acp")))
    (should (code-agent-acp-backend-needs-authenticate backend))
    (should (functionp
...
closure off a created backend instance."
  (dolist (method '(openai-api-key codex-api-key chatgpt))
    (let* ((code-agent-acp-codex-auth-method method)
...
(ert-deftest test-acp-backend-supports-p ()
  ...
  (let ((backend (code-agent-acp-backend--create)))
    ...
    (should (code-agent-backend-supports-p backend :persistent-client))
    (should-not (code-agent-backend-supports-p backend :structured-messages))
    (should-not (code-agent-backend-supports-p backend :session-resume))))
...
(ert-deftest test-acp-backend-ready-p ()
  ...
  (let ((backend (code-agent-acp-backend--create)))
    (should (code-agent-backend-ready-p backend))
    (setf (code-agent-acp-backend-active-query backend) t)
    (should-not (code-agent-backend-ready-p backend))))
...
(ert-deftest test-acp-backend-classify-error ()
  ...
  (let ((backend (code-agent-acp-backend--create)))
    (should (eq (code-agent-backend-classify-error backend "auth failed")
                'auth))
    (should (eq (code-agent-backend-classify-error backend "session expired")
                'session-expired))
    (should (eq (code-agent-backend-classify-error backend "ECONNREFUSED")
                'network))
    (should (eq (code-agent-backend-classify-error backend "something else")
                'unknown))
    (should (eq (code-agent-backend-classify-error backend nil)
                'unknown))))
...
(ert-deftest test-acp-notification-handler-agent-message ()
  ...
  (let* ((tokens nil)
         (backend (code-agent-acp-backend--create)))
    (setf (code-agent-acp-backend-callbacks backend)
          (list :on-token (lambda (text) (push text tokens))))
    (code-agent-acp--handle-notification
     backend
...
(ert-deftest test-acp-extract-content-text ()
  ...
  (should (equal (code-agent-acp--extract-content-text
                  [((type . "text") (text . "line 1"))
                   ((type . "text") (text . "line 2"))])
                 "line 1\nline 2"))
  ;; Empty
  (should-not (code-agent-acp--extract-content-text nil))
  (should-not (code-agent-acp--extract-content-text [])))

;;; Tests: Permission handler

(ert-deftest test-acp-permission-handler-auto-approve ()
  "When `code-agent-acp-auto-approve' is t, handler picks the first option."
  (let* ((tokens nil)
         (backend (code-agent-acp-backend--create))
         (code-agent-acp-auto-approve t))
    (setf (code-agent-acp-backend-callbacks backend)
          (list :on-token (lambda (text) (push text tokens))))
    (let* ((sent-data nil)
      ...
      (unwind-protect
          (progn
            (setf (code-agent-acp-backend-client backend) client)
            (code-agent-acp--handle-request
             backend
...
(ert-deftest test-acp-permission-handler-prompts-user-by-default ()
  "With `code-agent-acp-auto-approve' nil (default), prompts via completing-read."
  (let* ((backend (code-agent-acp-backend--create))
         (code-agent-acp-auto-approve nil)
         (prompt-seen nil))
...
(ert-deftest test-acp-backend-handshake-and-prompt ()
  ...
  (let* ((tokens nil)
         (completed nil)
         (backend (code-agent-acp-opencode-create :cwd "/tmp"))
         ;; Create a fake client
         (proc (start-process "test-acp-hs" nil "sleep" "10"))
         (client (code-agent-jsonrpc-make-client proc)))
    (setf (code-agent-acp-backend-client backend) client)
    (setf (code-agent-acp-backend-initialized backend) t)
    (setf (code-agent-acp-backend-session-id backend) "test-session-42")
...
(ert-deftest test-acp-backend-cleanup ()
  ...
  (let ((backend (code-agent-acp-backend--create
                  :session-key "test")))
    (setf (code-agent-acp-backend-session-id backend) "sid")
    (setf (code-agent-acp-backend-initialized backend) t)
    (setf (code-agent-acp-backend-active-query backend) t)
...
(ert-deftest test-acp-backend-verbose-buffer-returns-nil ()
  ...
  (let ((backend (code-agent-acp-backend--create :session-key "test")))
...
(ert-deftest test-acp-verbose-log-writes-to-shared-buffer ()
  ...
  (let* ((sk "/tmp/test::verbose-test")
         (backend (code-agent-acp-backend--create :session-key sk)))
    ...
    (unwind-protect
        (progn
          ...
          (let ((content (with-current-buffer
                             (gethash sk code-agent--session-verbose-buffers)
                           (buffer-string))))
...
(ert-deftest test-acp-verbose-completion-writes-duration ()
  ...
  (let* ((sk "/tmp/test::verbose-complete")
         (backend (code-agent-acp-backend--create :session-key sk)))
    (code-agent--get-session-verbose-buffer sk)
    (setf (code-agent-acp-backend-prompt-start-time backend)
    ...
    (unwind-protect
        (progn
          ...
          (should-not (code-agent-acp-backend-prompt-start-time backend)))
      (when-let* ((buf (gethash sk code-agent--session-verbose-buffers)))
        (kill-buffer buf))
      (remhash sk code-agent--session-verbose-buffers))))
           ...
           '((jsonrpc . "2.0") (method . "session/update")
             (params (update (sessionUpdate . "usage_update")
                             (usage (inputTokens . 1500)
                                    (outputTokens . 300))))))
          (should (= (code-agent-acp-backend-input-tokens backend) 1500))
          (should (= (code-agent-acp-backend-output-tokens backend) 300)))
      (when-let* ((buf (gethash sk code-agent--session-verbose-buffers)))
        (kill-buffer buf))
      (remhash sk code-agent--session-verbose-buffers))))
          ...
          (code-agent-acp--handle-notification
           ...
           '((jsonrpc . "2.0") (method . "session/update")
             (params (update (sessionUpdate . "usage_update")
                             ...
                             (used . 28000)
                             (size . 200000)))))
          (should (= (code-agent-acp-backend-cost-amount backend) 0.0042))
          (should (equal (code-agent-acp-backend-cost-currency backend) "USD"))
          (should (= (code-agent-acp-backend-context-used backend) 28000))
          (should (= (code-agent-acp-backend-context-size backend) 200000)))
      (when-let* ((buf (gethash sk code-agent--session-verbose-buffers)))
        (kill-buffer buf))
      (remhash sk code-agent--session-verbose-buffers))))
...
(ert-deftest test-acp-verbose-completion-shows-cost-and-tokens ()
  ...
  (let* ((sk "/tmp/test::verbose-cost")
    ...
    (setf (code-agent-acp-backend-prompt-start-time backend)
          (- (float-time) 3.0))
    (setf (code-agent-acp-backend-cost-amount backend) 0.0042)
    (setf (code-agent-acp-backend-cost-currency backend) "USD")
    (setf (code-agent-acp-backend-input-tokens backend) 1200)
    (setf (code-agent-acp-backend-output-tokens backend) 340)
    (unwind-protect
        (progn
          (code-agent-acp--verbose-completion backend)
          (let ((content (with-current-buffer
...
(ert-deftest test-acp-fs-write-creates-file ()
  ...
  (let* ((backend (code-agent-acp-backend--create))
         ...
         (code-agent-acp-fs-write-root nil))
...
(ert-deftest test-acp-fs-write-respects-root-sandbox ()
  "With `code-agent-acp-fs-write-root' set, writes outside it are rejected."
  (let* ((backend (code-agent-acp-backend--create))
         (sandbox (make-temp-file "acp-sandbox-" t))
         (outside (make-temp-file "acp-outside-" nil ".txt"))
         (code-agent-acp-fs-write-root sandbox))
    (delete-file outside)
    (unwind-protect
        (let* ((sent-data nil)
...
(ert-deftest test-acp-backend-persistence-slots ()
  ...
  (let* ((saved nil)
    ...
    (should (equal (code-agent-acp-backend-initial-session-id backend)
                   "prior-sid-123"))
    (should (functionp (code-agent-acp-backend-save-session-id-fn backend)))
    (funcall (code-agent-acp-backend-save-session-id-fn backend) "new-sid")
...
callbacks, losing its completion/error notification forever."
  (let* ((backend (code-agent-acp-backend--create))
         ...
         (complete-2 nil))
    (setf (code-agent-acp-backend-active-query backend) t)
    (setf (code-agent-acp-backend-callbacks backend) sentinel-cb)
    (unwind-protect
        (progn
          (code-agent-backend-query
           backend "second"
          ...
          (should (eq (code-agent-acp-backend-callbacks backend)
                      sentinel-cb))
...
(ert-deftest test-acp-resume-or-new-falls-back-to-new ()
  ...
  (let* ((saved-ids nil)
         ...
         (new-called nil)
         (ready-called nil))
    (code-agent--get-session-verbose-buffer "resume-test")
    (cl-letf (((symbol-function 'code-agent-acp--load-session)
               (lambda (_b _sid _on-ok on-err)
                 (setq load-called t)
                 (funcall on-err "session not found")))
              ((symbol-function 'code-agent-acp--new-session)
               (lambda (backend on-ok _on-err)
                 (setq new-called t)
                 (setf (code-agent-acp-backend-session-id backend) "fresh-sid")
                 (funcall on-ok))))
      (unwind-protect
          (progn
            (code-agent-acp--resume-or-new
             backend
        ...
        (when-let* ((buf (gethash "resume-test"
          ...
          (kill-buffer buf))
        (remhash "resume-test" code-agent--session-verbose-buffers)))))
...
(ert-deftest test-acp-sanitize-agent-string-strips-controls ()
  ...
  (should (equal (code-agent-acp--sanitize-agent-string
                  "Read \x1b[2Jfile")
                 "Read [2Jfile"))
  (should (equal (code-agent-acp--sanitize-agent-string "foo\nbar\tbaz")
                 "foobarbaz"))
  (should (equal (code-agent-acp--sanitize-agent-string nil) "")))

(ert-deftest test-acp-sanitize-agent-string-truncates ()
  "Long titles are truncated to `code-agent-acp-permission-title-max'."
  (let* ((code-agent-acp-permission-title-max 20)
         (input (make-string 100 ?x))
         (out (code-agent-acp--sanitize-agent-string input)))
  ...
  (let* ((sandbox (make-temp-file "acp-sandbox-" t))
    ...
    (unwind-protect
        (progn
          ...
          ;; the sandbox.
          (should-not (code-agent-acp--path-inside-p link sandbox))
          ;; A non-symlinked path inside the sandbox IS inside.
          (let ((legit (expand-file-name "file.txt" sandbox)))
            (should (code-agent-acp--path-inside-p legit sandbox))))
    ...
    (unwind-protect
        (progn
          ...
          (let ((victim (expand-file-name "pwn.txt" sibling)))
            (should-not (code-agent-acp--path-inside-p victim base))))
...
(ert-deftest test-acp-fs-read-sandbox-denies-outside-roots ()
  "With `code-agent-acp-fs-read-roots' set, reads outside are blocked."
  (let* ((sandbox (make-temp-file "acp-read-sbox-" t))
         ...
         (code-agent-acp-fs-read-roots (list sandbox)))
    (unwind-protect
        (progn
          (with-temp-file inside-file (insert "ok content"))
          (should (code-agent-acp--read-allowed-p inside-file))
          (should-not (code-agent-acp--read-allowed-p outside-file)))
...
(ert-deftest test-acp-fs-read-sandbox-nil-allows-all ()
  "Nil `code-agent-acp-fs-read-roots' permits reading any readable file."
  (let* ((tmp (make-temp-file "acp-read-all-" nil ".txt" "x"))
         (code-agent-acp-fs-read-roots nil))
    (unwind-protect
        (should (code-agent-acp--read-allowed-p tmp))
      (delete-file tmp))))
...
(ert-deftest test-acp-format-tokens-compact ()
  ...
  (should (equal (code-agent-acp--format-tokens-compact 500) "500"))
  (should (equal (code-agent-acp--format-tokens-compact 1200) "1.2k"))
  (should (equal (code-agent-acp--format-tokens-compact 15000) "15.0k"))
  (should (equal (code-agent-acp--format-tokens-compact 1500000) "2m")))

(provide 'test-code-agent-acp)
;;; test-code-agent-acp.el ends here
