;;; test-claude-agent-mock.el --- Mock CLI integration tests for claude-agent -*- lexical-binding: t; -*-
...
Mock slow-response scenario keeps process alive long enough to cancel."
  ...
  (let ((state nil)
    ...
    (should (test-claude-wait-until
             (lambda ()
               (not (claude-agent--query-active-p state)))
             5))))
...
Verifies the mock output is parsed into a complete result-message struct."
  ...
  (let ((result-msg nil)
    ...
    (should (claude-agent-result-message-session-id result-msg))
    (should (numberp (claude-agent-result-message-duration-ms result-msg)))
    (should (> (claude-agent-result-message-duration-ms result-msg) 0))
    (should (claude-agent-result-message-usage result-msg))
    ;; Usage should have input/output tokens
    (let ((usage (claude-agent-result-message-usage result-msg)))
...
(ert-deftest test-mock-abnormal-exit-detection ()
  ...
  (let ((claude-agent-auto-recovery t))
    ...
    (let ((state (claude-agent--make-process-state :session-id "test-uuid")))
    ...
    (let ((state (claude-agent--make-process-state :session-id "test-uuid" :got-result t)))
    ...
    ;; Recovery disabled
    (let ((claude-agent-auto-recovery nil)
          (state (claude-agent--make-process-state :session-id "test-uuid")))
      (should-not (claude-agent--is-abnormal-exit-p "killed: 9" state)))))
...
(provide 'test-claude-agent-mock)
;;; test-claude-agent-mock.el ends here
