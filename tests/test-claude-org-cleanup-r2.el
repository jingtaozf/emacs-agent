;;; test-claude-org-cleanup-r2.el --- Round 2 cleanup tests  -*- lexical-binding: t; -*-
...
(defun test-r2--setup-session (session-key &rest props)
  ...
  (while props
    (claude-org--session-put session-key (car props) (cadr props))
...
BUG B1: cancel only sets :busy nil, leaves :recovering stale."
  ...
  (test-r2--with-org-buffer
    ...
    (let ((sk "test::cancel-recovering"))
      ...
      (should (claude-org--session-get sk :recovering))
      (should (claude-org--session-get sk :last-assistant-query-id))
      ...
      (claude-org--cleanup-session sk "cancelled" t)
      ;; :recovering should be cleared
      (should-not (claude-org--session-get sk :recovering))
      ;; :last-assistant-query-id should be cleared
      (should-not (claude-org--session-get sk :last-assistant-query-id)))))
...
BUG B2: cancel uses clear-queue which doesn't free markers."
  ...
  (test-r2--with-org-buffer
    ...
    (let* ((sk "test::cancel-queue-markers")
      ...
      (should-not (claude-org--session-get sk :pending-queue)))))

(ert-deftest test-r2-cancel-function-uses-cleanup-session ()
  "The actual claude-org-cancel function should use cleanup-session.
Verify by checking that :recovering is cleared (which only reset-session-state does)."
  ...
  (test-r2--with-org-buffer
    ...
    (let ((sk (claude-org--current-session-key)))
      ...
      (claude-org--session-put sk :recovering t)
      (claude-org--session-put sk :backend 'mock-backend)
      (claude-org--session-put sk :query-id "qid-f19-3")
      (claude-org--session-put sk :marker (copy-marker (point-min)))
      ...
      (should-not (claude-org--session-get sk :recovering))
      ;; :busy should be nil
      (should-not (claude-org--session-get sk :busy)))))
...
uses the returned position in the narrowed restriction — goto-char clamps."
  ...
  (test-r2--with-org-buffer
    ...
    (let ((sk "test::narrow-insert"))
      ...
      (let ((result (claude-org--insert-at-response sk "Token from Claude")))
        (should result))
...
(ert-deftest test-r2-find-response-end-widened ()
  ...
  (test-r2--with-org-buffer
    ...
    (let ((end-pos (claude-org--find-response-end-by-query-id "qid-f20-2")))
      (should end-pos)
        ...
        (with-current-buffer buf
          ...
          (let ((sk "test::kill-cancel"))
            (test-r2--setup-session sk
              :busy t
              :marker (copy-marker (point-min))))
          ;; Install the buffer-kill hook (simulates claude-org-mode enable)
...
BUG A6: run-at-time timer is never stored or cancelled."
  ...
  (test-r2--with-org-buffer
    ...
    (let* ((sk "test::loop-timer")
      ...
      (should-not (claude-org--session-get sk :loop-timer)))))
