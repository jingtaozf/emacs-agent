...
(ert-deftest e2e-pi-multiblock--same-session-and-restart ()
  "Run 3 ai blocks under the same workspace heading.

  Block 1 → block 2: must share the SAME Pi PID (continuous session).
  Block 2 → block 3 (after kill): Pi must respawn (DIFFERENT PID),
  responses still arrive, session-key unchanged.

If this test hangs at block 2 with :busy stuck, the 2026-05-25
regression is the underlying cause — block 1's agent_end did not
clear the session's :busy flag, and block 2's dispatch was queued
instead."
  ...
  (test-mb--with-fresh-fixture buf
    (let ((sk (test-mb--session-key buf)))
      ...
      (let ((resp1 (test-mb--response-text buf "pi-mb-1"))
        ...
        ;; ---- Block 2 (must share PID with block 1) ----
        ...
        (let ((resp2 (test-mb--response-text buf "pi-mb-2"))
          ...
          (should (string-match-p "ACK_TWO" resp2))
          ;; SAME subprocess — proves continuous session.
          (should (= pid1 pid2)))
        ...
        ;; ---- Block 3 (respawn — different PID, same session-key) ----
        ...
        (let ((resp3 (test-mb--response-text buf "pi-mb-3"))
          ...
          (should (string-match-p "ACK_THREE" resp3))
