;;; test-code-agent-org-loop.el --- Tests for loop feature -*- lexical-binding: t -*-
...
EXTRA-BINDINGS is a list of additional cl-letf bindings."
  ...
  `(let ((code-agent-org-complete-hook nil))
     (cl-letf (((symbol-function 'code-agent-org--stop-spinner) #'ignore)
               ...
               ((symbol-function 'code-agent-org--get-exec-status-for-session)
                (lambda (&rest _) "executing"))
               ((symbol-function 'code-agent-org--unregister-active-query) #'ignore)
               ((symbol-function 'code-agent-org--dequeue-block) (lambda (&rest _) nil))
               ,@extra-bindings)
       ,@body)))
...
(ert-deftest test-code-agent-org-loop-header-args-parsing ()
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (search-forward "Test")
    (let ((args (code-agent-org--get-block-header-args)))
      (should (equal (plist-get args :loop) "3"))
      (should (equal (plist-get args :interval) "2")))))

(ert-deftest test-code-agent-org-loop-max-conversion ()
...
(ert-deftest test-code-agent-org-maybe-continue-loop-condition ()
...
(ert-deftest test-code-agent-org-handle-complete-marker-free-condition ()
...
(ert-deftest test-code-agent-org-loop-session-state ()
...
(ert-deftest test-code-agent-org-loop-state-reset-on-reexecute ()
...
from iteration 1, not continue from where it left off."
  ...
  (let ((test-sessions (make-hash-table :test 'equal))
    ...
    ;; Simulate what code-agent-org-execute does on re-execution
...
(ert-deftest test-code-agent-org-loop-continuation-logic ()
...
(ert-deftest test-code-agent-org-handle-complete-continues-loop ()
...
(ert-deftest test-code-agent-org-handle-complete-stops-at-loop-max ()
...
(ert-deftest test-code-agent-org-handle-complete-preserves-marker-for-interval ()
...
(ert-deftest test-code-agent-org-loop-iteration-section-not-inside-src-block ()
  ...
  (let* ((test-buffer (generate-new-buffer "*test-iter-pos*"))
    ...
    (unwind-protect
        (with-current-buffer test-buffer
          ...
          (with-loop-test-mocks
            (((symbol-function 'code-agent-org--send-request) #'ignore))
...
(ert-deftest test-code-agent-org-loop-iteration-has-query-id-property ()
...
(ert-deftest test-code-agent-org-section-level-captured-early ()
...
(provide 'test-code-agent-org-loop)
;;; test-code-agent-org-loop.el ends here
