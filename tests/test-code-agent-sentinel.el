;;; test-code-agent-sentinel.el --- Process sentinel tests -*- lexical-binding: t; -*-
...
;; but we only load code-agent.org in agent-unit tests. Define it here
...
Sentinel should detect abnormal exit and clean up state."
  :tags '(:unit :sentinel)
  (let ((code-agent-auto-recovery nil)  ; disable recovery for clean test
        (completed nil)
        (error-received nil)
        (state nil))
    (setq state
    ...
    (should-not (code-agent--process-state-got-result state))
...
Uses rate-limited scenario: exits with code 1 after error result message."
  ...
  (let ((completed nil)
    ...
    (should (test-claude-wait-until
             (lambda () (and state (code-agent--process-state-closed state)))
             10))
...
vs OOM kill (code 137) vs permission denied (code 126)."
  ...
  (let* ((received-err nil)
    ...
    (code-agent--sentinel-handle-normal-exit
     state "exited abnormally with code 42\n")
    ;; Error plist must exist
    (should received-err)
    ;; Must be a proper error plist
    (should (eq 'code-agent-process-error (car received-err)))
  ...
  (let* ((state (code-agent--make-process-state
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'code-agent--insert-recovery-message)
                     (lambda (_state _event) nil))
                    ((symbol-function 'code-agent--kill-child-processes)
                     (lambda (_pid) nil))
                    ((symbol-function 'code-agent-registry-cleanup-process)
                     (lambda (_state) nil))
...
Invokes code-agent--process-sentinel directly with a mock process
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'code-agent--kill-child-processes)
            ...
            (code-agent--process-sentinel proc "finished\n"))
          ;; State must be closed
          (should (code-agent--process-state-closed state))
          ;; Ready must be nil
          (should-not (code-agent--process-state-ready state))
...
Invokes code-agent--process-sentinel directly with exit code 1."
  :tags '(:unit :sentinel :regression)
  (let* ((completed nil)
         (complete-err nil)
         (state (code-agent--make-process-state
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'code-agent--kill-child-processes)
                     (lambda (_pid) nil)))
            (code-agent--process-sentinel proc "exited abnormally with code 1\n"))
          ;; State must be closed
          (should (code-agent--process-state-closed state))
          ...
          (should (eq 'code-agent-process-error (car complete-err))))
      (when (process-live-p proc) (delete-process proc)))))
  ...
  (let* ((completed nil)
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'code-agent--kill-child-processes)
                     (lambda (_pid) nil)))
            (code-agent--process-sentinel proc "hangup\n"))
          ;; State must be closed
          (should (code-agent--process-state-closed state))
...
must produce an error plist for signal events."
  :tags '(:unit :sentinel :regression)
  (let* ((code-agent-auto-recovery nil)  ; disable recovery
         (completed nil)
         (complete-err nil)
         (state (code-agent--make-process-state
                 :json-buffer ""
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'code-agent--kill-child-processes)
                     (lambda (_pid) nil)))
            (code-agent--process-sentinel proc "killed: 9\n"))
          ;; State must be closed
          (should (code-agent--process-state-closed state))
          ;; Complete callback must have fired with error
          (should completed)
