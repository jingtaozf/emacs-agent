;;; test-claude-org-wrong-position-repro.el --- Tests for wrong insert position bugs -*- lexical-binding: t; -*-
...
;; - `claude-org--find-response-boundary' (uses org-current-level at wrong point)
;; - `claude-org--find-last-ai-output-sibling' (builds regex with wrong star count)
;; - `claude-org--insert-at-response' (stops at generated headings within response)
;; - `claude-org--find-response-insert-point' (returns wrong position for queued blocks)
;;
...
that level instead."
  ...
  (with-temp-buffer
    ...
    (let ((boundary-from-instruction (claude-org--find-response-boundary)))
      ...
      (beginning-of-line)
      (let ((boundary-from-response (claude-org--find-response-boundary)))
...
So at level 3, it looks for *** headings with :ai_output:, missing ** ones."
  ...
  (with-temp-buffer
    ...
    (re-search-forward "^\\*\\* Instruction 1$")
    (beginning-of-line)
    (let ((result-from-correct-level (claude-org--find-last-ai-output-sibling)))
    ...
    (let ((level-2-pattern (format "^\\*\\{%d\\} .*:%s:" 2 claude-org-output-tag))
          (level-3-pattern (format "^\\*\\{%d\\} .*:%s:" 3 claude-org-output-tag)))
...
instead of at the END of the response section."
  ...
  (with-temp-buffer
    ...
    (let ((state (claude-org--make-session-state
                  ...
                  :section-level 1)))
      (puthash "test-session" state claude-org--sessions))

    ;; Insert new text via insert-at-response
    (claude-org--insert-at-response "test-session" "NEW TOKEN")
...
same-level sibling, not extend to point-max."
  ...
  (with-temp-buffer
    ...
    (let ((boundary (claude-org--find-response-boundary)))
...
be safe, but we verify the trailing $ anchor behavior."
  ...
  (with-temp-buffer
    ...
    (let ((pos1 (claude-org--find-response-by-query-id "20260204-120000-abc")))
      (should pos1)
      (goto-char pos1)
      (should (looking-at "\\*\\* Response 1")))

    ;; Search for the longer ID - should find Response 2
    (let ((pos2 (claude-org--find-response-by-query-id "20260204-120000-abcd")))
      (should pos2)
      (goto-char pos2)
    ...
    (let ((pos1 (claude-org--find-response-by-query-id "20260204-120000-abc"))
          (pos2 (claude-org--find-response-by-query-id "20260204-120000-abcd")))
...
not Block A's."
  ...
  (with-temp-buffer
    ...
    (claude-org--insert-at-response "test-session" "NEW-B-TOKEN")
...
insert-at-response should insert after :END: in the blank area."
  ...
  (with-temp-buffer
    ...
    ;; Insert text
    (claude-org--insert-at-response "test-session" "FIRST-TOKEN")
...
(provide 'test-claude-org-wrong-position-repro)

;;; test-claude-org-wrong-position-repro.el ends here
