;;; test-claude-org-content-loss-repro.el --- Tests reproducing SDD content loss -*- lexical-binding: t; -*-
  ...
  (with-temp-buffer
    ...
    (let ((section-level 0)
      ...
      (let* ((claude-text "* Summary\nContent here.\n** Details\nMore details.\n")
             (normalized (claude-org--normalize-headers-in-text claude-text target-level)))
...
text that disrupts the tree structure."
  ...
  (with-temp-buffer
    ...
    (let* ((session-key "/tmp/test-content-loss.org")
      ...
      (let ((raw (claude-org--session-get session-key :section-level)))
        (should (null raw))
...
(provide 'test-claude-org-content-loss-repro)
;;; test-claude-org-content-loss-repro.el ends here
