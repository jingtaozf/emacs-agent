;;; test-code-agent-org-heading-level.el --- Tests for heading level normalization -*- lexical-binding: t; -*-
...
(ert-deftest test-header-normalization-uses-correct-level ()
  ...
  (with-temp-buffer
    ...
    (let ((section-level (code-agent-org--get-section-level)))
      ...
      (let ((target-level (1+ section-level)))
        ...
        (let ((input-text "* Header\nContent\n** Subheader\n"))
          ;; Normalize: * -> ***, ** -> ****
          (let ((result (code-agent-org--normalize-headers-in-text input-text target-level)))
...
(ert-deftest test-create-response-header-wrong-level-bug ()
  "Test that demonstrates the bug in code-agent-org--create-response-section-header.
The function uses the cursor position to determine heading level, but when called
from within code-agent-org-execute, the cursor has already moved to insert-point
which may be INSIDE a previous Response section's nested headings."
  ...
  (with-temp-buffer
    ...
    ;; But code-agent-org--find-response-insert-point returns AFTER the Response subtree
    ;; Problem: if we're still conceptually "inside" the nested heading...
    (let ((insert-point (point)))
      ;; Go back to AI block to get correct level
      (goto-char (point-min))
      (re-search-forward "query")
      (let ((correct-level (code-agent-org--get-section-level)))
        (should (= 3 correct-level))  ; Correct: level 3 from Task One
        ;; Now go to insert-point (simulating what code-agent-org-execute does)
        (goto-char insert-point)
        ;; BUG: At this position, org-back-to-heading finds **** Nested Heading
        (let ((buggy-level (code-agent-org--get-section-level)))
...
positioned inside those nested headings, the wrong level is detected."
  ...
  (with-temp-buffer
    ...
    ;; Note: code-agent-org should create Response at level 3 as sibling
    (insert "*** Response 1 (2025-01-13 08:00) :ai_output:\n\n")
...
not recalculated at call time."
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (re-search-forward "query")
    (let ((correct-section-level (code-agent-org--get-section-level)))
...
(ert-deftest test-section-level-deeply-nested ()
  ...
  (with-temp-buffer
    ...
    (should (= 5 (code-agent-org--get-section-level)))))

(provide 'test-code-agent-org-heading-level)
;;; test-code-agent-org-heading-level.el ends here
