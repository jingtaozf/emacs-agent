;;; test-code-agent-org-pre-completion.el --- Tests for F4: Pre-completion hook -*- lexical-binding: t; -*-
...
;;; Test 1: code-agent-org-query-complete-hook exists as defcustom

(ert-deftest test-pre-completion-hook-is-defcustom ()
  "code-agent-org-query-complete-hook exists as a defcustom.
FIX: Define (defcustom code-agent-org-query-complete-hook nil ...) in code-agent-org.org."
  :tags '(:unit :fast :stable :pre-completion)
  (should (custom-variable-p 'code-agent-org-query-complete-hook))
  (should (null (default-value 'code-agent-org-query-complete-hook))))

(ert-deftest test-pre-completion-hook-is-buffer-local ()
  "code-agent-org-query-complete-hook is buffer-local.
FIX: Call (make-variable-buffer-local 'code-agent-org-query-complete-hook)."
  :tags '(:unit :fast :stable :pre-completion)
  (with-temp-buffer
    (add-hook 'code-agent-org-query-complete-hook #'ignore nil t)
    (should (local-variable-p 'code-agent-org-query-complete-hook))
    (with-temp-buffer
      (should (null code-agent-org-query-complete-hook)))))
...
FIX: Add (run-hook-with-args 'code-agent-org-query-complete-hook ...) in handle-complete."
  :tags '(:unit :fast :stable :pre-completion)
  (let ((calls '()))
    (with-temp-buffer
      ...
      (let* ((session-key "test-session")
        ...
        (run-hook-with-args 'code-agent-org-query-complete-hook session-key "q-123")
        (should (equal calls '(("test-session" "q-123"))))))))

;;; Test 3: code-agent-org-append-to-response utility

(ert-deftest test-pre-completion-append-to-response ()
  "code-agent-org-append-to-response inserts text at end of response section.
FIX: Implement (defun code-agent-org-append-to-response (session-key query-id text) ...)."
  :tags '(:unit :fast :stable :pre-completion)
  (with-temp-buffer
    (org-mode)
    (setq-local code-agent-org--sessions (make-hash-table :test 'equal))
...
(ert-deftest test-pre-completion-append-preserves-window-start ()
  "code-agent-org-append-to-response preserves window scroll position."
  :tags '(:unit :fast :stable :pre-completion)
  (with-temp-buffer
    (org-mode)
    (setq-local code-agent-org--sessions (make-hash-table :test 'equal))
...
(provide 'test-code-agent-org-pre-completion)
;;; test-code-agent-org-pre-completion.el ends here
