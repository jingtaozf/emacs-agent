...
(defun claude-ide--register-external-tool (tool-def)
  "Register an external TOOL-DEF.
TOOL-DEF is an alist with keys: name, description, handler, inputSchema.
HANDLER should be a function that takes (params session) and returns MCP result."
  (let* ((name (alist-get 'name tool-def))
         (existing (seq-find (lambda (t) (equal (alist-get 'name t) name))
                             claude-ide--external-tools)))
    (when existing
      (setq claude-ide--external-tools (delete existing claude-ide--external-tools)))
    (push tool-def claude-ide--external-tools)))

(defun claude-ide--unregister-external-tool (name)
  "Unregister the external tool with NAME."
  (setq claude-ide--external-tools
        (seq-remove (lambda (t) (equal (alist-get 'name t) name))
                    claude-ide--external-tools)))
...
(defun claude-ide--status-message (msg)
...
(defun claude-ide--get-session (key)
  "Get the Claude IDE session for KEY, or nil if not found."
  (gethash key claude-ide--sessions))
#+END_SRC
