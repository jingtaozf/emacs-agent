...
The MCP tools served over the WebSocket are organised in this file as:
...
The IDE server is started per cmux workspace, not per Emacs session.
The catalysts:
...
(=python/claude_agent/claude_workspace.py=) does *not* manage the IDE
...
(=claude-agent-ide.org=), which surfaces selection / open editors /
diagnostics into the trace metadata for post-hoc analysis.

** Migration History

Origin: =monet.el= by Stephen Molitor. Adapted into this file for the
claude-agent SDK in 2026-03 — the =monet-= prefix renamed to
=claude-ide-= and the file converted to literate org. The protocol
specifics are unchanged; bug-for-bug compatibility with monet's
WebSocket + MCP wire was the design intent so existing clients (Claude
Code's =--ide= flag) continue to work without server-side changes.
The companion test (=test-diff-visibility.el=) also moved from monet
into =tests/test-claude-ide-diff.el= with the same prefix rename.
