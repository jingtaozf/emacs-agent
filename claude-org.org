...
Limited by `claude-org-history-max-entries'."
  ...
  (let* ((entries-with-id (cl-remove-if-not
                           ...
                           claude-org--block-history))
         ;; Filter to only entries whose CUSTOM_ID still exists in buffer
         (valid-entries (cl-remove-if-not
                         (lambda (entry)
                           (claude-org--custom-id-exists-p
                            (plist-get (cdr entry) :custom-id)))
                         entries-with-id))
         (limited-entries (seq-take valid-entries claude-org-history-max-entries)))
    (mapcar (lambda (entry)
