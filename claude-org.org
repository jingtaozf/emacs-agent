...
Returns t if block was added, nil if already queued or currently running."
  (let* ((queue (claude-org--session-get session-key :pending-queue))
         ...
         (new-pos (and new-marker (marker-position new-marker)))
         ;; Check if same block is currently running
         (running-pos (and running-marker (marker-position running-marker)))
         ...
         (in-queue (and new-pos
                        (cl-some (lambda (queued)
                                   (let ((m (plist-get queued :marker)))
                                     (and m (= (marker-position m) new-pos))))
                                 queue))))
    (cond
     (is-running nil)  ; Cannot queue the running block
     (in-queue nil)    ; Already queued
     (t
...
(defun claude-org--insert-error (session-key error-msg)
  ...
  (let ((queue-count (claude-org--queue-count session-key)))
    (when (> queue-count 0)
  ...
  (let* ((content (claude-org--get-block-content))
    ...
    (when (claude-org--session-get session-key :busy)
      (let ((block-info (list :custom-id custom-id
        ...
        (if (claude-org--queue-block session-key block-info)
            (message "Queued [%d]. Will run after current query completes."
                     (claude-org--queue-count session-key))
          (message "Block already queued [%d in queue]."
                   (claude-org--queue-count session-key)))
        (cl-return-from claude-org-execute nil)))
    ;; Record execution timestamp BEFORE creating response section
  ...
  (let* ((session-key (or session-key-override (claude-org--current-session-key)))
    ...
    (let ((org-section-name
      ...
      (lexical-let ((buf source-buf)
        ...
        ;; Pass :cli-path and :path-mappings for Docker mode support
        ;; Pass :source-location-fn and :source-location-args for session manager navigation
