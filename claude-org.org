...
Claudeception-style post-execution hook for extracting skills and rules.
Users manually add `claude-org-extract-knowledge' to `claude-org-complete-hook'.

#+NAME: knowledge-extraction-config
#+BEGIN_SRC elisp
(defcustom claude-org-loop-extraction-timing 'after-all
  "When to extract skills/rules during loop execution.
- `after-all': Extract once after all iterations complete (recommended)
- `after-each': Extract after each iteration"
  :type '(choice (const :tag "After all iterations" after-all)
                 (const :tag "After each iteration" after-each))
  :group 'claude-org)
#+END_SRC
...
Claudeception-style post-execution analysis for extracting reusable skills and rules.
Users manually add the hook to enable extraction after AI block execution.

** Extraction Timing

#+BEGIN_SRC elisp
(defun claude-org--should-extract-now-p (session-key)
  "Determine if skill extraction should run for SESSION-KEY.
For loops: respects `claude-org-loop-extraction-timing'.
Returns non-nil if extraction should proceed."
  (let ((loop-max (or (claude-org--session-get session-key :loop-max) 1))
        (loop-current (or (claude-org--session-get session-key :loop-current) 1)))
    (if (> loop-max 1)
        ;; Loop execution
        (pcase claude-org-loop-extraction-timing
          ('after-all (>= loop-current loop-max))
          ('after-each t)
          (_ nil))
      ;; Single execution
      t)))
...
#+BEGIN_SRC elisp
(defun claude-org--get-current-section-level ()
  "Get the org heading level of the section containing point.
Returns the level (1, 2, 3, etc.) or 1 if not in a section."
  (save-excursion
    (condition-case nil
        (progn
          (org-back-to-heading t)
          (org-outline-level))
      (error 1))))  ; Default to level 1 if not in a heading

(defun claude-org--run-knowledge-extraction (session-key)
  "Execute knowledge extraction (skills and rules) for SESSION-KEY.
Uses same session via :resume option to preserve conversation context.
Loads prompt from prompts/skills/claudeception.org."
  (let* ((marker (claude-org--session-get session-key :marker))
         (buffer (when marker (marker-buffer marker)))
         (prompt-file (expand-file-name "skills/claudeception.org"
                                        claude-org-prompts-directory))
         (prompt (claude-org--load-prompt-file prompt-file)))
    (cond
     ((not prompt)
      (message "Claudeception prompt not found at %s" prompt-file))
     ((not (and buffer (buffer-live-p buffer)))
      (message "Buffer no longer exists for skill extraction"))
     (t
      ;; Get SDK UUID from session state (set during query completion)
      ;; This is more reliable than reading from org property which may be from older session
      (let* ((sdk-uuid (or (claude-org--session-get session-key :sdk-uuid)
                           ;; Fallback to org property if not in session state
                           (with-current-buffer buffer
                             (save-excursion
                               (goto-char marker)
                               (claude-org--get-sdk-uuid)))))
             ;; Get current section level to create subsection at correct depth
             (section-level (with-current-buffer buffer
                              (save-excursion
                                (goto-char marker)
                                (claude-org--get-current-section-level))))
             ;; Extraction section should be a child of current section
             (extraction-level (1+ section-level))
             (extraction-stars (make-string extraction-level ?*)))
        ;; Insert extraction header at correct level
        (with-current-buffer buffer
          (save-excursion
            (goto-char marker)
            (insert (format "\n\n-----\n%s Skill/Rule Extraction\n\n" extraction-stars))
            (set-marker marker (point))))
        ;; Warn if no SDK UUID - extraction needs conversation context
        (unless sdk-uuid
          (message "Warning: No SDK UUID for extraction - starting fresh session"))
        ;; Create new marker for extraction (don't reuse session marker)
        (let* ((extract-marker (with-current-buffer buffer
                                 (copy-marker marker)))
               ;; Build options: use --continue instead of --resume for stability
               ;; --resume can fail with error_during_execution due to pending state
               ;; --continue is safer as it continues the most recent conversation
               (options (when sdk-uuid
                          (claude-agent-options :continue-conversation t))))
          ;; Use lexical-let to capture variables for async callbacks
          (lexical-let ((extract-marker extract-marker)
                        (session-key session-key))
            ;; Send extraction query
            (claude-agent-query
             prompt
             :session-key session-key
             :options options
             :on-token (lambda (token)
                         (when (and extract-marker (marker-buffer extract-marker))
                           (with-current-buffer (marker-buffer extract-marker)
                             (save-excursion
                               (goto-char extract-marker)
                               (insert token)
                               (set-marker extract-marker (point))))))
             :on-complete (lambda (result)
                            (claude-org--extraction-complete session-key result extract-marker))
             :on-error (lambda (err)
                         (let ((msg (claude-org--extract-error-message err)))
                           (cond
                            ;; Session expired - skip silently
                            ((claude-agent--session-expired-p msg)
                             (message "Extraction skipped: session expired")
                             (when (and extract-marker (marker-buffer extract-marker))
                               (with-current-buffer (marker-buffer extract-marker)
                                 (save-excursion
                                   (goto-char extract-marker)
                                   (insert "/Session expired - extraction skipped/")))))
                            ;; Execution error (tool failure) - skip gracefully
                            ((string-match-p "Execution error\\|error_during_execution" msg)
                             (message "Extraction skipped: execution error during resume")
                             (when (and extract-marker (marker-buffer extract-marker))
                               (with-current-buffer (marker-buffer extract-marker)
                                 (save-excursion
                                   (goto-char extract-marker)
                                   (insert "/Extraction skipped (session resume failed)/")))))
                            ...
                            (t
                             (when (and extract-marker (marker-buffer extract-marker))
                               (with-current-buffer (marker-buffer extract-marker)
                                 (save-excursion
                                   (goto-char extract-marker)
                                   (insert (format "\n/Extraction error: %s/" msg))))))))
                         (when extract-marker
                           (set-marker extract-marker nil)))))))))))

(defun claude-org--extraction-complete (session-key result extract-marker)
  "Handle extraction completion for SESSION-KEY with RESULT.
EXTRACT-MARKER is freed after completion."
  (when (and extract-marker (marker-buffer extract-marker))
    (with-current-buffer (marker-buffer extract-marker)
      (save-excursion
        (goto-char extract-marker)
        (insert "\n"))))
  ;; Free the extraction marker
  (when extract-marker
    (set-marker extract-marker nil))
  ;; Log completion
  (message "Skill/rule extraction completed for %s"
...
#+BEGIN_SRC elisp
(defun claude-org-extract-knowledge (session-key result status)
  "Run knowledge extraction (skills and rules) after execution completes.
SESSION-KEY identifies the session, RESULT is the completion result,
STATUS is `completed' or `error'.

Add to `claude-org-complete-hook' to enable:

  (add-hook \\='claude-org-complete-hook
            #\\='claude-org-extract-knowledge)

For loops, respects `claude-org-loop-extraction-timing'.
Prompt loaded from prompts/skills/claudeception.org."
  (when (and (eq status 'completed)
             (claude-org--should-extract-now-p session-key))
    (claude-org--run-knowledge-extraction session-key)))
#+END_SRC
...
(defun claude-org-cancel ()
  "Cancel the current session's active query and stop any loop."
  (interactive)
  (let* ((session-key (claude-org--current-session-key))
