...
IMPORTANT: This is called from async error handler where cursor may have moved.
All cursor-dependent operations (org-entry-get, collect-session-context) must
be done within the marker's buffer with cursor positioned at marker location.

If marker is invalid, recovery will fail gracefully and show error to user."
  ...
  (let ((marker (claude-org--session-get session-key :marker))
    ...
    ;; Check marker validity - if invalid, we can't recover
    (if (not (claude-org--marker-valid-p marker))
        (progn
          (message "Claude: Cannot recover session - marker invalid")
      ...
      ;; Marker is valid - proceed with recovery
      (with-current-buffer (marker-buffer marker)
        ;; Position cursor at marker for all org-entry-get calls to work correctly
        (save-excursion
          (goto-char marker)
          ;; Insert recovery message
          (insert (format "\n%s\n\n" msg))
          (set-marker marker (point)))
        ;; Clear old session UUID from org property
        (claude-org--clear-sdk-uuid)
        ;; Collect context while still positioned in correct buffer
        ;; NOTE: collect-session-context uses org-entry-get which depends on cursor position
        ;; but uses save-excursion internally, so being in the right buffer is sufficient
        (let* ((context (save-excursion
                          (goto-char marker)
                          (claude-org--collect-session-context)))
          ...
          ;; Retry with recovery prompt, passing session-key explicitly
...
(defun claude-org--find-instruction-number ()
  "Extract instruction number from current or ancestor heading.
Walks up the heading hierarchy to find the nearest Instruction N heading.
Returns nil if not within an Instruction heading tree."
  (save-excursion
    (when (ignore-errors (org-back-to-heading t))
      ;; Check current heading first
      (if (looking-at "^\\*+ Instruction \\([0-9]+\\)")
...
(defun claude-org--handle-complete (session-key result)
  ...
  ;; Update header line
  (claude-org--refresh-header-line)
  ;; Run completion hook (BEFORE freeing marker - hooks may need it)
  (run-hook-with-args 'claude-org-complete-hook session-key result 'completed)
  ;; Free marker to prevent memory leak (unless loop continues)
  ;; Done AFTER hooks so extraction can use the marker
  ...
  (let* ((content (claude-org--get-block-content))
    ...
    (let ((timestamp (format-time-string "%Y-%m-%d %H:%M:%S")))
      ...
      (let ((insert-point (claude-org--find-response-insert-point))
        ...
        ;; (org-back-to-heading would find the new Response heading otherwise)
        (let ((exec-status-marker (save-excursion
          ...
          (let ((query-ctx (when instr-num
                             (claude-agent-make-query-context
                              ...
                              :loop-max loop-max
                              :custom-id custom-id))))
            ;; First iteration's section header already created above
      ...
      (lexical-let ((buf source-buf)
        ...
        (if claude-org-persistent-sessions
            ...
            (let ((client (claude-org--get-or-create-persistent-client session-key options)))
              ...
              ;; Store client state for cancellation
