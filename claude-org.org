...
Or without instr-num: \"** Response (2025-01-13 08:00) :ai_output:\""
  (let* ((level (claude-org--get-section-level))
    ...
    (format "%s Response %d%s (%s) :%s:\n\n"
            stars instr-num loop-suffix display-time claude-org-output-tag)))
#+END_SRC
...
ITERATION is current iteration number (1-based).
MAX is total iterations.
INSTR-NUM is the instruction number for section naming.
...
so this works correctly even when the user is editing other parts of the file."
  (let ((marker (claude-org--session-get session-key :marker))
        (level (or (claude-org--session-get session-key :section-level)
                   (claude-org--get-section-level))))
    (when (and marker (marker-buffer marker) instr-num)
      (with-current-buffer (marker-buffer marker)
        (save-excursion
          (goto-char marker)
          (let ((stars (make-string level ?*)))
