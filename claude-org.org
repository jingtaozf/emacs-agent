...
Returns:
  ...
  (let* ((queue (claude-org--session-get session-key :pending-queue))
         ...
         ;; Use marker-valid-p to safely check positions
         (new-pos (and (claude-org--marker-valid-p new-marker)
                       (marker-position new-marker)))
         ;; Check if same block is currently running (only if markers valid)
         (running-pos (and (claude-org--marker-valid-p running-marker)
                           (marker-position running-marker)))
         (is-running (and new-pos running-pos (= new-pos running-pos)))
         ;; Check if same block is already in queue
         (in-queue (and new-pos
                        (cl-some (lambda (queued)
                                   (let ((m (plist-get queued :marker)))
                                     (and (claude-org--marker-valid-p m)
                                          (= (marker-position m) new-pos))))
                                 queue))))
    (cond
  ...
  (let ((total 0))
    (when (hash-table-p claude-org--sessions)
      (maphash (lambda (_key state)
                 (cl-incf total (length (plist-get state :pending-queue))))
               claude-org--sessions))
    total))
#+END_SRC
...
(defun claude-org--set-exec-status-for-session (session-key status)
  "Set exec STATUS for SESSION-KEY using marker or custom-id fallback.
Returns non-nil if status was successfully set."
  (let ((exec-marker (claude-org--session-get session-key :exec-status-marker))
        (custom-id (claude-org--session-get session-key :custom-id)))
    (cond
     ;; Primary: use exec-status-marker if valid
     ((and exec-marker (marker-buffer exec-marker))
      (claude-org--set-exec-status status exec-marker)
      t)
     ;; Fallback: use custom-id to find heading
     (custom-id
      (save-excursion
...
(defun claude-org--get-exec-status-for-session (session-key)
  "Get current exec status for SESSION-KEY using marker or custom-id fallback."
  (let ((exec-marker (claude-org--session-get session-key :exec-status-marker))
        (custom-id (claude-org--session-get session-key :custom-id)))
    (cond
     ((and exec-marker (marker-buffer exec-marker))
      (with-current-buffer (marker-buffer exec-marker)
        (save-excursion
          (goto-char exec-marker)
...
terminal state immediately, without waiting for the CLI process to exit."
  (when-let* ((process-state (claude-org--session-get session-key :process-state)))
    (setf (claude-agent--process-state-closed process-state) t)
    (when-let* ((req-id (claude-agent--process-state-request-id process-state)))
...
CRITICAL: This function MUST clear :recovering flag to prevent stuck state."
  (claude-org--session-put session-key :busy nil)
  (claude-org--session-put session-key :recovering nil)  ; CRITICAL: Clear recovering flag
  (claude-org--session-put session-key :last-assistant-query-id nil)
  (claude-org--stop-spinner session-key)
  (let ((marker (claude-org--session-get session-key :marker)))
    (when (claude-org--marker-valid-p marker)
  ...
  (let* ((content (claude-org--get-block-content))
    ...
    (let ((timestamp (format-time-string "%Y-%m-%d %H:%M:%S")))
      ...
      (let ((insert-point (claude-org--find-response-insert-point))
        ...
        ;; Capture exec-status-marker at instruction heading BEFORE inserting Response section
        (let ((exec-status-marker (save-excursion
                                    (org-back-to-heading t)
                                    (copy-marker (point)))))
          ...
          (let ((query-id (claude-org--generate-query-id))
            ...
            (claude-org--session-put session-key :exec-status-marker exec-status-marker)
...
(defun claude-org-cancel-all ()
  ...
  (let ((cancelled 0)
    ...
    (maphash (lambda (key state)
               ;; Clear queue
               (let ((q-count (length (plist-get state :pending-queue))))
                 (when (> q-count 0)
                   (cl-incf queued-cleared q-count)
                   (plist-put state :pending-queue nil)))
               ;; Cancel active query
               (when (plist-get state :busy)
                 (when-let* ((process-state (plist-get state :process-state)))
                   (claude-agent-query-interrupt process-state)
      ...
      (maphash (lambda (key state)
		 (push (list key
			     ...
			     (claude-org--format-elapsed (plist-get state :start-time))
                             (length (plist-get state :pending-queue)))
		       sessions))
	       claude-org--sessions))
    (if sessions
