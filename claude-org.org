...
(defcustom claude-org-recent-blocks-limit 30
  "Maximum number of recent blocks to show in `claude-org-jump-to-ai-block'.
When nil, show all blocks (up to `claude-org-history-max-entries')."
  :type '(choice (const :tag "No limit" nil)
                 (integer :tag "Maximum count"))
  :group 'claude-org)
...
Limited by `claude-org-recent-blocks-limit'."
  (claude-org--ensure-history-loaded)
  (let* ((valid-entries (cl-remove-if-not
                         (lambda (entry) (plist-get (cdr entry) :custom-id))
                         claude-org--block-history))
         ;; Apply limit if set
         (limited-entries (if claude-org-recent-blocks-limit
                              (seq-take valid-entries claude-org-recent-blocks-limit)
                            valid-entries)))
    (mapcar (lambda (entry)
