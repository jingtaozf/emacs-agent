      ...
      (if (not response-pos)
        ...
        (save-excursion
          ...
          (let ((new-query-id (claude-org--generate-query-id)))
            ...
            (claude-org--session-put session-key :needs-message-separator nil)
  ...
  (let ((query-id (claude-org--session-get session-key :query-id)))
    (when query-id
      (let* ((target-level (1+ (or (claude-org--session-get session-key :section-level) 0)))
        ...
        ;; Prepend newline separator before new assistant message if needed
        (when (and (> (length final-text) 0)
                   (claude-org--session-get session-key :needs-message-separator))
          (claude-org--session-put session-key :needs-message-separator nil)
...
(defun claude-org-define-templates (templates)
  "Set `claude-org-templates' to TEMPLATES.
TEMPLATES is an alist of (NAME . VALUE) where VALUE is a string or function symbol."
  (setq claude-org-templates templates))
...
(defcustom claude-org-templates
  (claude-org--load-templates-from-file
   (or claude-org-templates-file
       (claude-org--templates-file-path)))
  "Alist of templates for AI block insertion.
Each entry is (NAME . TEMPLATE) where:
- NAME is a string displayed in the completion menu
- TEMPLATE is either:
  - A string to insert verbatim
  - A function (no args) returning a string

Default values are loaded from `claude-org-templates-file' (or the built-in
`claude-org-templates.el').  To customize, either:
  1. Edit `claude-org-templates.el' in the package directory
  2. Set `claude-org-templates-file' to your own file
  3. Set this variable directly in your init file"
  :type '(alist :key-type (string :tag "Template name")
                :value-type (choice (string :tag "Template text")
                                    (function :tag "Generator function")))
  :group 'claude-org)
#+END_SRC
...
(defun claude-org--template-annotation (name)
  ...
  (let ((template (cdr (assoc name claude-org-templates))))
    (cond
     ((functionp template)
      "  [function]")
     ((stringp template)
...
Shows completion menu with available templates from `claude-org-templates'.
If not inside an AI block, creates one first."
  ...
  (let* ((names (mapcar #'car claude-org-templates))
...
Extracts session ID from result messages and stores it."
  ...
  (when (claude-agent-assistant-message-p msg)
    ;; Mark that next token should be preceded by a newline separator.
    ;; Token callback fires before on-message for the SAME message,
    ;; so this flag set by message N will affect message N+1's tokens.
    (claude-org--session-put session-key :needs-message-separator t)
    (let ((marker (claude-org--session-get session-key :marker)))
...
Uses query-id based architecture with marker for queue duplicate detection."
  ...
  (unless (claude-org--session-get session-key :busy)
    (when-let ((next-block (claude-org--dequeue-block session-key)))
      ...
      (let ((insert-point (claude-org--find-response-insert-point))
        ...
        (let ((exec-status-marker (save-excursion
          ...
          (let ((query-id (claude-org--generate-query-id))
            ...
            ;; Reset recovery count and message separator flag for new query
