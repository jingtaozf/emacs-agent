  ...
  (let ((marker (claude-org--session-get session-key :marker)))
    (when (and marker (marker-buffer marker))
      (with-current-buffer (marker-buffer marker)
        ...
        ;; Insert iteration marker
        (claude-org--insert-iteration-marker iteration max)
    ...
    (let ((insert-point (claude-org--find-block-end))
      ...
      (let ((query-ctx (when instr-num
                         (claude-agent-make-query-context
                          ...
                          :loop-max loop-max
                          :custom-id custom-id))))
        ;; Insert iteration marker if looping
        (when (> loop-max 1)
          (claude-org--insert-iteration-marker 1 loop-max))
...
#+BEGIN_SRC elisp
(defun claude-org--insert-iteration-marker (current max)
  "Insert iteration marker before response.
CURRENT is the current iteration (1-based), MAX is total iterations."
  (let ((marker (claude-org--session-get (claude-org--current-session-key) :marker)))
    (when marker
      (save-excursion
        (goto-char marker)
        (insert (format "\n----- Iteration %d/%d -----\n\n" current max))
...
(defun claude-org--insert-loop-complete (completed max)
  "Insert completion marker after loop ends.
COMPLETED is iterations completed, MAX is total."
  (let ((marker (claude-org--session-get (claude-org--current-session-key) :marker)))
    (when marker
      (save-excursion
        (goto-char marker)
        (insert (format "\n----- Loop complete: %d/%d iterations -----\n" completed max))))))
#+END_SRC
