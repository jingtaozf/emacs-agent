  ...
  (let* ((session-key (claude-org--current-session-key))
    ...
    (if (and process-state (claude-org--session-get session-key :busy))
        (progn
          ...
          ;; Mark process state closed and unregister from active queries
          ;; so mode-line and *Claude Queries* buffer reflect cancellation
          ;; immediately (without waiting for sentinel to fire)
    ...
    (maphash (lambda (key state)
               ...
               (when (plist-get state :busy)
                 (let ((process-state (plist-get state :process-state)))
                   (when process-state
                     (claude-agent-query-interrupt process-state)
                     ;; Mark closed and unregister from active queries
                     (setf (claude-agent--process-state-closed process-state) t)
                     (when-let* ((req-id (claude-agent--process-state-request-id process-state)))
                       (claude-agent--unregister-query req-id))
