...
Implementation tag passes MAX_RETRIES configuration dynamically.

#+BEGIN_SRC elisp
(cl-defmethod claude-org-tag-prompt ((tag (eql 'implementation)) context)
  "Implementation tag: combine Summary with MAX_RETRIES configuration.
MAX_RETRIES is sourced from (in priority order):
1. :max-retries header argument on ai block
2. :MAX_RETRIES org property on Features section or parent
3. Default: 3"
  (let* ((max-retries (or (plist-get context :max-retries)
                          (save-excursion
                            (when-let ((prop (org-entry-get nil "MAX_RETRIES" t)))
                              (string-to-number prop)))
                          3))
         (static-prompt (claude-org--extract-summary-section
                         (expand-file-name "tags/implementation.org"
                                           claude-org-prompts-directory))))
    (concat static-prompt
            "\n\n** Runtime Configuration\n\n"
            (format "| Parameter | Value |\n|-----------|-------|\n| MAX_RETRIES | %d |\n" max-retries)
            "\nMAX_RETRIES: Maximum test-fix cycles before reporting failure.\n")))
#+END_SRC
...
(defun claude-org--build-behavior-context (&optional header-args)
  "Build context plist for tag prompt generation.
Returns plist with :file-path, :sdd-root, :session-id, :current-tags.
If HEADER-ARGS is provided, extracts :max-retries and other relevant args."
  (let ((context (list :file-path (buffer-file-name)
                       :sdd-root (claude-org--find-sdd-root)
                       :session-id (claude-org--get-session-id)
                       :current-tags (claude-org--get-current-tags))))
    ;; Extract :max-retries from header arguments if present
    (when-let ((max-retries (plist-get header-args :max-retries)))
      (setq context (plist-put context :max-retries
                               (if (stringp max-retries)
                                   (string-to-number max-retries)
                                 max-retries))))
    context))
...
headings matching `claude-org-auto-title-heading-pattern' using Haiku."
  ...
  (let* ((content (claude-org--get-block-content))
    ...
    (let ((insert-point (claude-org--find-block-end))
      ...
      ;; Pass instruction number and CUSTOM_ID for session manager
      (claude-org--send-request content instr-num custom-id)
...
#+BEGIN_SRC elisp
(defun claude-org--send-request (prompt &optional instr-num custom-id)
  "Send PROMPT to Claude via claude-agent.
INSTR-NUM is the instruction number for display in session manager (optional).
CUSTOM-ID is the CUSTOM_ID of the instruction heading for navigation (optional).
