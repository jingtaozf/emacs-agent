  ...
  (let ((custom-id (claude-org--find-instruction-custom-id)))
    ...
    (when custom-id
      ...
      (let ((existing (cl-find-if (lambda (e)
                                    (string= (plist-get (cdr e) :custom-id) custom-id))
                                  claude-org--block-history)))
        (if existing
    ...
    (let ((insert-point (claude-org--find-block-end))
      ...
      (save-excursion
        ...
        (when (and claude-org-auto-create-response-section instr-num)
          (let* ((level (claude-org--get-section-level))
                 (stars (make-string level ?*)))
            (if (> loop-max 1)
                ;; Loop: include iteration in section name "Response N(1/M)"
                (insert stars " Response " (number-to-string instr-num)
                        "(1/" (number-to-string loop-max) ")"
                        " :" claude-org-output-tag ":\n\n")
              ;; Non-loop: original format "Response N"
              (insert stars " Response " (number-to-string instr-num)
                      " :" claude-org-output-tag ":\n\n"))))
      ...
      (let ((query-ctx (when instr-num
                         (claude-agent-make-query-context
                          ...
                          :loop-max loop-max
                          :custom-id custom-id))))
        ;; First iteration's section header already created above with Response N(1/M)
