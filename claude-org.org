...
(defun claude-org--cleanup-stale-sessions ()
  "Remove stale entries from sessions hash table.
Called after code reload to clean up entries with dead buffers."
  (when (and (boundp 'claude-org--sessions)
             (hash-table-p claude-org--sessions))
    (let ((stale-keys '()))
      (maphash
       (lambda (key state)
         (when (or (not (listp state))
                   (let ((buf (plist-get state :source-buffer)))
                     (and buf (not (buffer-live-p buf)))))
           (push key stale-keys)))
       claude-org--sessions)
